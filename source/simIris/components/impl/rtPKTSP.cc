...
#ifndef  _rtPKTSP_cc_INC
#define  _rtPKTSP_cc_INC
...
RtPKTSP::RtPKTSP ()
{
    name = "RtPKTSP" ;
    ticking = false;
}  /* -----  end of method RtPKTSP::RtPKTSP  (constructor)  ----- */
...
void
RtPKTSP::find_neighbors(uint p, uint v, uint cr, uint bs)
{
    ports =p;
    vcs =v;
    credits =cr;
...
RtPKTSP::set_fault(uint p)
{
	...
	_DBG("; Permenant Fault Injection in_port %d ",p);

#endif
	return ;
}
void
RtPKTSP::set_temp_fault(uint p,uint type)
{
...
void
RtPKTSP::remove_fault(uint p,uint type)
{
	if(type==6000)
		fault_outports.erase(remove(fault_outports.begin(),fault_outports.end(),p),fault_outports.end());// erase/remove idiom
	else if(type==9000)
...
void
RtPKTSP::set_temp_opfault(uint p, double t, ullint d)
{
...
void
RtPKTSP::set_temp_ipfault(uint p, double t, ullint d)
{
    IrisEvent* event = new IrisEvent();
    event->type = FAULT_INJECT_EVENT;
...
void
RtPKTSP::handle_fault_remove_event(IrisEvent* e )
{
...
void
RtPKTSP::handle_fault_inject_event(IrisEvent* e )
{
#ifdef _DEBUG_FI
	if(e->src_id==6000)
	{
...
void
RtPKTSP::init (uint p, uint v, uint cr, uint bs)
{
    ports =p;
    vcs =v;
    credits =cr;
...
void
RtPKTSP::set_grid_x_location( uint port, uint x_node, uint value)
{
...
void
RtPKTSP::set_grid_y_location( uint port, uint y_node, uint value)
{
...
void
RtPKTSP::process_event ( IrisEvent* e )
{
    switch(e->type)
    {
        case LINK_ARRIVAL_EVENT:
#ifdef _DEBUG_ROUTER
        	_DBG_NOARG("; RtPKTSP::LINK_ARRIVAL_EVENT ;");
#endif
            handle_link_arrival_event(e);
            break;
        case TICK_EVENT:
#ifdef _DEBUG_ROUTER
        	_DBG_NOARG("; RtPKTSP::TICK_EVENT ;");
#endif
            handle_tick_event(e);
            break;
        case DETECT_DEADLOCK_EVENT:
#ifdef _DEBUG_ROUTER
        	_DBG_NOARG("; RtPKTSP::DETECT_DEADLOCK_EVENT ;");
#endif
            handle_detect_deadlock_event(e);
            break;
        case FAULT_INJECT_EVENT:
#ifdef _DEBUG_ROUTER
        	_DBG_NOARG("; RtPKTSP::FAULT_INJECT_EVENT ;");
#endif
            handle_fault_inject_event(e);
            break;
        case FAULT_REMOVE_EVENT:
#ifdef _DEBUG_ROUTER
        	_DBG_NOARG("; RtPKTSP::FAULT_REMOVE_EVENT ;");
#endif
            handle_fault_remove_event(e);
            break;
        case RESET_STAT_EVENT:
#ifdef _DEBUG_ROUTER
        	_DBG_NOARG("; RtPKTSP::RESET_STAT_EVENT ;");
#endif
        	handle_reset_stat_event(e);
            break;

        default:
            _DBG(";RtPKTSP:: Unk event exception %d", e->type);
            break;
    }
    return ;
}		/* -----  end of function RtPKTSP::process_event  ----- */

void 
RtPKTSP::handle_detect_deadlock_event(IrisEvent* e )
{
    for ( uint i=0; i<ports; i++)
        for ( uint j=0; j<vcs; j++)
        {
...
double RtPKTSP::get_average_packet_latency()
{
	return ((stat_total_packet_latency+0.0)/stat_packets);
}

double RtPKTSP::get_last_flit_out_cycle()
{
	return last_flit_out_cycle;
}

double RtPKTSP::get_flits_per_packet()
{
	return (stat_flits+0.0)/(stat_packets) ;
}

double RtPKTSP::get_buffer_occupancy()
{
	return stat_buffer_occupancy;	
}

double RtPKTSP::get_packets_drop()
{
	return stat_pkts_drop;
}

double RtPKTSP::get_pshf_created()
{
	return stat_pshf_created;
}
double RtPKTSP::get_pstf_created()
{
	return stat_pstf_created;
}
double RtPKTSP::get_swa_fail_msg_ratio()
{
	return stat_swa_fail_msg_ratio;	
}

double RtPKTSP::get_swa_load()
{
	return stat_swa_load;
}

double RtPKTSP::get_vca_fail_msg_ratio()
{
	return stat_vca_fail_msg_ratio;
}

double RtPKTSP::get_vca_load()
{
	return stat_vca_load;
}
 
double RtPKTSP::get_stat_packets()
{
	return stat_packets;	
}

double RtPKTSP::get_stat_flits()
{
	return stat_flits;
}

double RtPKTSP::get_stat_ib_cycles()
{
	return stat_ib_cycles;	
}

double RtPKTSP::get_stat_rc_cycles()
{
	return stat_rc_cycles;
}

double RtPKTSP::get_stat_vca_cycles()
{
	return stat_vca_cycles;
}
        
double RtPKTSP::get_stat_swa_cycles()
{
	return stat_swa_cycles;
}
 
double RtPKTSP::get_stat_st_cycles()
{
...
void
RtPKTSP::handle_link_arrival_event ( IrisEvent* e )
{
    ...
    delete e; e=NULL;
    return ;
}		/* -----  end of function RtPKTSP::handle_link_arrival_event  ----- */

void
RtPKTSP::handle_reset_stat_event( IrisEvent* e)
{
    /* init the countes */
    stat_packets = 0;
    stat_flits = 0;
...
void
RtPKTSP::do_switch_allocation_m() //Yi: m=modified
{
    bool was_swa_active = false; /* flag to make sure swa accesses are not double counted */
...
void
RtPKTSP::do_virtual_channel_allocation()
{
/* flag to count accesses. In effect multiple w
 * inners can be chosen for one access of the arbiter */
bool was_vca_active = false;
...
void
RtPKTSP::request_switch_allocation()
{
    for( uint i=0; i<(ports*vcs); i++)
...
void
RtPKTSP::handle_tick_event ( IrisEvent* e )
{
    ticking = false;
    //do_switch_traversal();
    do_st();
...
}		/* -----  end of function RtPKTSP::handle_input_arbitration_event  ----- */

string
RtPKTSP::toString() const
{
    stringstream str;
    str << "RtPKTSP"
        << "\t addr: " << address
...
void
RtPKTSP::send_credit_back(uint i)
{
        ...
        if( input_buffer_state[i].pipe_stage == IB )
        {
        	...
        	if ( input_buffer_state[i].possible_oports.size() == 2 )
        	{
                ...
                for ( uint i=0; i<no_adaptive_aports; i++ )
                {
                    ...
                    if( static_cast<RtPKTSP*>(static_cast<GenericLink*>(output_connections[porta])->output_connection)->neighbors[rc_port] != NULL )  // should check if exits
                    {
                    	a_aoc.push_back(rc_port);
                     temp=static_cast<RtPKTSP*>(static_cast<GenericLink*>(output_connections[porta])->output_connection)->get_sum_bf(rc_port);
                ...
                for ( uint i=0; i<no_adaptive_bports; i++ )
                {
                    ...
                    if( static_cast<RtPKTSP*>(static_cast<GenericLink*>(output_connections[portb])->output_connection)->neighbors[rc_port] != NULL )  // should check if exits
                    {
                        b_aoc.push_back(rc_port);
                        temp=static_cast<RtPKTSP*>(static_cast<GenericLink*>(output_connections[portb])->output_connection)->get_sum_bf(rc_port);

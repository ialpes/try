 ...
 * =====================================================================================
 *
 *       Filename:  genericInterfaceNBpr.cc
 *
...
#ifndef  _genericInterfaceNBpr_cc_INC
#define  _genericInterfaceNBpr_cc_INC

#include	"genericInterfaceNBpr.h"
...
GenericInterfaceNBpr::GenericInterfaceNBpr()
{
    name = "GenericInterfaceNBpr";
    ticking =false;
}		/* -----  end of function GenericInterfaceNBpr::GenericInterfaceNBpr  ----- */
...
void
GenericInterfaceNBpr::setup (uint v, uint cr, uint bs)
{
    vcs =v;
    credits = cr;
    buffer_size = bs;
...
*/
    ...
    return;
}		/* -----  end of function GenericInterfaceNBpr::setup  ----- */
void
GenericInterfaceNBpr::handle_check_event (IrisEvent* e)
{
	for(uint i=0; i<vcs; i++)
	{
...
string
GenericInterfaceNBpr::toString () const
{
    stringstream str;
    str << "GenericInterfaceNBpr: "
        << "\t VC: " << out_packets.size()
        ...
        ;
    return str.str();
}		/* -----  end of function GenericInterfaceNBpr::toString  ----- */

void
GenericInterfaceNBpr::set_no_credits( int cr )
{
}		/* -----  end of function GenericInterfaceNBpr::toString  ----- */

uint
GenericInterfaceNBpr::get_no_credits() const
{
    return credits;
}		/* -----  end of function GenericInterfaceNBpr::toString  ----- */

void
GenericInterfaceNBpr::set_no_vcs( uint cr )
{
}		/* -----  end of function GenericInterfaceNBpr::toString  ----- */

void
GenericInterfaceNBpr::set_buffer_size( uint cr )
{
}		/* -----  end of function GenericInterfaceNBpr::toString  ----- */
void
GenericInterfaceNBpr::set_buffer_size( uint vcs, uint bs )
{
    in_buffer.resize(vcs, bs);
    out_buffer.resize(vcs, bs);
}		/* -----  end of function GenericInterfaceNBpr::toString  ----- */
void
GenericInterfaceNBpr::process_event(IrisEvent* e)
{
    switch(e->type)
    {
        ...
        case READY_EVENT:
#ifdef _DEBUG
            _DBG("; GenericInterfaceNBpr::READY_EVENT ; time:%f", Simulator::Now());
#endif
...
#ifdef _DEBUG
            _DBG("; GenericInterfaceNBpr::LINK_ARRIVAL_EVENT ; time:%f", Simulator::Now());
#endif
...
#ifdef _DEBUG
            _DBG("; GenericInterfaceNBpr::NEW_PACKET_EVENT ; time:%f", Simulator::Now());
#endif
        ...
        case TICK_EVENT:
#ifdef _DEBUG
            _DBG("; GenericInterfaceNBpr::TICK_EVENT ; time:%f", Simulator::Now());
#endif
            handle_tick_event(e);
            break;
...
#ifdef _DEBUG
            _DBG("; GenericInterfaceNBpr::RESET_STAT_EVENT time:%f", Simulator::Now());
#endif
        ...
        case VS_TICK_EVENT:
#ifdef _DEBUG
            _DBG("; GenericInterfaceNBpr::VS_TICK_EVENT time:%f", Simulator::Now());
#endif
        ...
        case CHECK_EVENT:
#ifdef _DEBUG
            _DBG("; GenericInterfaceNBpr::CHECK_EVENT time:%f", Simulator::Now());
#endif
...
}		/* -----  end of function GenericInterfaceNBpr::process_event ----- */

void
GenericInterfaceNBpr::handle_ready_event( IrisEvent* e)
{
...
if( in_ready[e->vc] )
{
    ...
    delete e;e=NULL;
    return;
}		/* -----  end of function GenericInterfaceNBpr::handle_ready_event  ----- */

void
GenericInterfaceNBpr::handle_link_arrival ( IrisEvent* e)
{
...
*/
        ...
        {
        	HeadFlit * hf = static_cast<HeadFlit*>(uptr->ptr);
        	if(hf->transaction_id==2000)
        	{
        		//for(uint j=0; j<hf->node_stamping.size();j++)
        			if(hf->node_stamping.size()>4)
        			{
        				//uint index=(hf->node_stamping.size()*4)/5;
        				uint index=(hf->node_stamping.size()*3)/4;
        				//uint index=(hf->node_stamping.size()*2)/3;
        				//uint index=(hf->node_stamping.size())-2;
        				uint op=hf->node_stamping.at(index);
        				uint node=hf->path.at(index);
        				//topology_ptr->routers[node]->set_temp_opfault(op,Simulator::Now()+1-0.1,1000);
        				//static_cast<Router*>(try_ptr)->set_temp_opfault(op,Simulator::Now()+1-0.1,1000);
        				IrisEvent* event = new IrisEvent();
        				        event->type = FAULT_INJECT_EVENT;
        				        event->src_id = 6000;
        				        event->dst_id = op;//faulty port
        				            event->time = 1000;//fault duration

        				        Simulator::Schedule( floor(Simulator::Now())+1, &NetworkComponent::process_event,
        				        		static_cast<NetworkComponent*>(ptr_router.at(node)), event);
        			}
...
*/
    ...
    if(!ticking)
    {
        ...
        new_event->vc = e->vc;
        Simulator::Schedule( floor(Simulator::Now())+1, &GenericInterfaceNBpr::process_event, this, new_event);
    }
    delete uptr; uptr=NULL;
    delete e; e=NULL;
    return;
}		/* -----  end of function GenericInterfaceNBpr::handle_link_arrival  ----- */

void
GenericInterfaceNBpr::handle_new_packet_event(IrisEvent* e)
{
...
//    delete pkt;
    ...
    delete e; e=NULL;
    return;
}		/* -----  end of function GenericInterfaceNBpr::handle_new_packet_event  ----- */

void
GenericInterfaceNBpr::handle_reset_stat_event( IrisEvent* e)
{
    /*  init stats */
    packets_out = 0;
    vs_packets_in = 0;
...
void
GenericInterfaceNBpr::handle_vs_tick_event(IrisEvent* e)
{
...
void
GenericInterfaceNBpr::handle_tick_event(IrisEvent* e)
{
    ticking = false;
...
#endif
    						in_packets[ch].add(ptr); // to get resource recycle
    						// drop pkt
#ifdef _DEBUG_PKT_SPLIT
    						ullint waiting_time = Simulator::Now() - in_packets[ch].in_packet_arrival;
    						_DBG("; pkt %lld is dropped at NI due to pseudo tail flit waiting time %lld",in_packets[ch].addr, waiting_time);
#endif
...
string
GenericInterfaceNBpr::print_stats()
{
    stringstream str;
...
ullint
GenericInterfaceNBpr::get_flits_in()
{
    return flits_in;
}

ullint
GenericInterfaceNBpr::get_flits_out()
{
    return flits_out;
}

ullint
GenericInterfaceNBpr::get_packets_out()
{
    return packets_out;
}
ullint
GenericInterfaceNBpr::get_vs_packets_in()
{
    return vs_packets_in;
}
ullint
GenericInterfaceNBpr::get_packets_in()
{
    return packets_in;
}

ullint
GenericInterfaceNBpr::get_packets()
{
    return packets_out + packets_in;
}
ullint
GenericInterfaceNBpr::get_packets_drop()
{
    return stat_pkts_drop;
}
#endif   /* ----- #ifndef _GenericInterfaceNBpr_cc_INC  ----- */

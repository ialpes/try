...
#ifndef  _rtPGCP_cc_INC
#define  _rtPGCP_cc_INC
...
RtPGCP::RtPGCP ()
{
	name = "RtPGCP" ;
	ticking = false;
}  /* -----  end of method RtPGCP::RtPGCP  (constructor)  ----- */
...
void
RtPGCP::find_neighbors(uint p, uint v, uint cr, uint bs)
{
	ports =p;
	vcs =v;
	credits =cr;
...
void
RtPGCP::set_temp_opfault(uint p, double t, ullint d)
{
...
void
RtPGCP::set_temp_ipfault(uint p, double t, ullint d)
{
	return ;
}

void
RtPGCP::handle_fault_inject_event(IrisEvent* e )
{
...
void
RtPGCP::init (uint p, uint v, uint cr, uint bs)
{
	ports =p;
	vcs =v;
	credits =cr;
...
void
RtPGCP::set_grid_x_location( uint port, uint x_node, uint value)
{
...
void
RtPGCP::set_grid_y_location( uint port, uint y_node, uint value)
{
...
void
RtPGCP::process_event ( IrisEvent* e )
{
	switch(e->type)
	{
	case LINK_ARRIVAL_EVENT:
#ifdef _DEBUG_ROUTER
		_DBG("; RtPGCP::LINK_ARRIVAL_EVENT ; time:%f", Simulator::Now());
#endif
		handle_link_arrival_event(e);
		break;
	case TICK_EVENT:
#ifdef _DEBUG_ROUTER
		_DBG("; RtPGCP::TICK_EVENT ; time:%f", Simulator::Now());
#endif
		handle_tick_event(e);
		break;
...
#ifdef _DEBUG_ROUTER
		_DBG("; RtPGCP::FAULT_INJECT_EVENT ; time:%f", Simulator::Now());
#endif
		handle_fault_inject_event(e);
		break;
	case FAULT_REMOVE_EVENT:
#ifdef _DEBUG_ROUTER
		_DBG("; RtPGCP::FAULT_REMOVE_EVENT ; time:%f", Simulator::Now());
#endif
		handle_fault_remove_event(e);
		break;
	case RESET_STAT_EVENT:
#ifdef _DEBUG_ROUTER
		_DBG("; RtPGCP::RESET_STAT_EVENT time:%f", Simulator::Now());
#endif
		handle_reset_stat_event(e);
		break;
	case DBAR_ARRIVAL_EVENT:
#ifdef _DEBUG_ROUTER
		_DBG("; RtPGCP::DBAR_ARRIVAL_EVENT time:%f", Simulator::Now());
#endif
		handle_dbar_arrival_event(e);
		break;

	default:
		_DBG(";RtPGCP:: Unk event exception %d", e->type);
		break;
	}
	return ;
}		/* -----  end of function RtPGCP::process_event  ----- */
void 
RtPGCP::handle_dbar_arrival_event(IrisEvent* e )
{
...
void
RtPGCP::handle_detect_deadlock_event(IrisEvent* e )
{
	for ( uint i=0; i<ports; i++)
		for ( uint j=0; j<vcs; j++)
		{
...
double RtPGCP::get_average_packet_latency()
{
	return ((stat_total_packet_latency+0.0)/stat_packets);
}

double RtPGCP::get_last_flit_out_cycle()
{
	return last_flit_out_cycle;
}

double RtPGCP::get_flits_per_packet()
{
	return (stat_flits+0.0)/(stat_packets) ;
}

double RtPGCP::get_buffer_occupancy()
{
	return stat_buffer_occupancy;	
}

double RtPGCP::get_packets_drop()
{
	return stat_pkts_drop;
}

double RtPGCP::get_swa_fail_msg_ratio()
{
	return stat_swa_fail_msg_ratio;	
}

double RtPGCP::get_swa_load()
{
	return stat_swa_load;
}

double RtPGCP::get_vca_fail_msg_ratio()
{
	return stat_vca_fail_msg_ratio;
}

double RtPGCP::get_vca_load()
{
	return stat_vca_load;
}

double RtPGCP::get_stat_packets()
{
	return stat_packets;	
}

double RtPGCP::get_stat_flits()
{
	return stat_flits;
}

double RtPGCP::get_stat_ib_cycles()
{
	return stat_ib_cycles;	
}

double RtPGCP::get_stat_rc_cycles()
{
	return stat_rc_cycles;
}

double RtPGCP::get_stat_vca_cycles()
{
	return stat_vca_cycles;
}

double RtPGCP::get_stat_swa_cycles()
{
	return stat_swa_cycles;
}

double RtPGCP::get_stat_st_cycles()
{
...
void
RtPGCP::handle_link_arrival_event ( IrisEvent* e )
{
	LinkArrivalData* data = static_cast<LinkArrivalData*>(e->event_data.at(0));
	...
	delete e;
	return ;
}		/* -----  end of function RtPGCP::handle_link_arrival_event  ----- */

void
RtPGCP::handle_reset_stat_event( IrisEvent* e)
{
	/* init the countes */
	stat_packets = 0;
	stat_flits = 0;
...
void
RtPGCP::do_switch_allocation_m() //Yi: m=modified
{
	bool was_swa_active = false; /* flag to make sure swa accesses are not double counted */
...
void
RtPGCP::do_virtual_channel_allocation()
{
	/* flag to count accesses. In effect multiple w
	 * inners can be chosen for one access of the arbiter */
	bool was_vca_active = false;
...
void
RtPGCP::request_switch_allocation()
{
	for( uint i=0; i<(ports*vcs); i++)
...
void
RtPGCP::handle_tick_event ( IrisEvent* e )
{
	ticking = false;
	//do_switch_traversal();
	do_st();
	...
	for( uint i=0; i<(ports*vcs); i++)
		if( input_buffer_state[i].pipe_stage == IB )
		{
			...
			//char c[]=str.str();
			_DBG("; handle_tick_event; %s", "metric_x");
			cerr << str_x.str();
			_DBG("; handle_tick_event; %s", "metric_y");
			cerr << str_y.str();
			//cerr <<endl;
#endif
							switch(temp_op)
...
}		/* -----  end of function RtPGCP::handle_input_arbitration_event  ----- */

string
RtPGCP::toString() const
{
	stringstream str;
	str << "RtPGCP"
			<< "\t addr: " << address
...
void
RtPGCP::send_credit_back(uint i)
{
		...
		if( input_buffer_state[i].pipe_stage == IB )
		{
			...
			if ( input_buffer_state[i].possible_oports.size() == 2 )
			{
				...
				for ( uint i=0; i<no_adaptive_aports; i++ )
				{
					...
					if( static_cast<RtPGCP*>(static_cast<GenericLink*>(output_connections[porta])->output_connection)->neighbors[rc_port] != NULL )  // should check if exits
					{
						a_aoc.push_back(rc_port);
						temp=static_cast<RtPGCP*>(static_cast<GenericLink*>(output_connections[porta])->output_connection)->get_sum_bf(rc_port);
				...
				for ( uint i=0; i<no_adaptive_bports; i++ )
				{
					...
					if( static_cast<RtPGCP*>(static_cast<GenericLink*>(output_connections[portb])->output_connection)->neighbors[rc_port] != NULL )  // should check if exits
					{
						b_aoc.push_back(rc_port);
						temp=static_cast<RtPGCP*>(static_cast<GenericLink*>(output_connections[portb])->output_connection)->get_sum_bf(rc_port);

...
#ifndef  _rtSPDBAR_cc_INC
#define  _rtSPDBAR_cc_INC
...
RtSPDBAR::RtSPDBAR ()
{
    name = "RtSPDBAR" ;
    ticking = false;
}  /* -----  end of method RtSPDBAR::RtSPDBAR  (constructor)  ----- */
...
void
RtSPDBAR::find_neighbors(uint p, uint v, uint cr, uint bs)
{
    ports =p;
    vcs =v;
    credits =cr;
...
void
RtSPDBAR::set_temp_fault(uint p,uint type)
{
...
void
RtSPDBAR::remove_fault(uint p,uint type)
{
	if(type==6000)
...
void
RtSPDBAR::set_temp_opfault(uint p, double t, ullint d)
{
...
void
RtSPDBAR::set_temp_ipfault(uint p, double t, ullint d)
{
    IrisEvent* event = new IrisEvent();
    event->type = FAULT_INJECT_EVENT;
...
void
RtSPDBAR::handle_fault_remove_event(IrisEvent* e )
{
...
void
RtSPDBAR::handle_fault_inject_event(IrisEvent* e )
{
#ifdef _DEBUG_FI
	if(e->src_id==6000)
	{
...
void
RtSPDBAR::init (uint p, uint v, uint cr, uint bs)
{
    ports =p;
    vcs =v;
    credits =cr;
...
void
RtSPDBAR::set_grid_x_location( uint port, uint x_node, uint value)
{
...
void
RtSPDBAR::set_grid_y_location( uint port, uint y_node, uint value)
{
...
void
RtSPDBAR::process_event ( IrisEvent* e )
{
    switch(e->type)
    {
        case LINK_ARRIVAL_EVENT:
#ifdef _DEBUG_ROUTER
        	_DBG_NOARG("; RtSPDBAR::LINK_ARRIVAL_EVENT ;");
#endif
            handle_link_arrival_event(e);
            break;
        case TICK_EVENT:
#ifdef _DEBUG_ROUTER
        	_DBG_NOARG("; RtSPDBAR::TICK_EVENT ;");
#endif
            handle_tick_event(e);
            break;
        case CHECK_EVENT:
#ifdef _DEBUG_ROUTER
        	_DBG_NOARG("; RtSPDBAR::CHECK_EVENT ;");
#endif
        	handle_check_event(e);
        	break;
        case DETECT_DEADLOCK_EVENT:
#ifdef _DEBUG_ROUTER
        	_DBG_NOARG("; RtSPDBAR::DETECT_DEADLOCK_EVENT ;");
#endif
            handle_detect_deadlock_event(e);
            break;
        case FAULT_INJECT_EVENT:
#ifdef _DEBUG_ROUTER
        	_DBG_NOARG("; RtSPDBAR::FAULT_INJECT_EVENT ;");
#endif
            handle_fault_inject_event(e);
            break;
        case FAULT_REMOVE_EVENT:
#ifdef _DEBUG_ROUTER
        	_DBG_NOARG("; RtSPDBAR::FAULT_REMOVE_EVENT ;");
#endif
            handle_fault_remove_event(e);
            break;
        case RESET_STAT_EVENT:
#ifdef _DEBUG_ROUTER
        	_DBG_NOARG("; RtSPDBAR::RESET_STAT_EVENT ;");
#endif
        	handle_reset_stat_event(e);
            break;

        default:
            _DBG(";RtSPDBAR:: Unk event exception %d", e->type);
            break;
    }
    return ;
}		/* -----  end of function RtSPDBAR::process_event  ----- */
void
RtSPDBAR::handle_check_event (IrisEvent* e)
{
...
void 
RtSPDBAR::handle_detect_deadlock_event(IrisEvent* e )
{
    for ( uint i=0; i<ports; i++)
        for ( uint j=0; j<vcs; j++)
        {
...
double RtSPDBAR::get_average_packet_latency()
{
	return ((stat_total_packet_latency+0.0)/stat_packets);
}

double RtSPDBAR::get_last_flit_out_cycle()
{
	return last_flit_out_cycle;
}

double RtSPDBAR::get_flits_per_packet()
{
	return (stat_flits+0.0)/(stat_packets) ;
}

double RtSPDBAR::get_buffer_occupancy()
{
	return stat_buffer_occupancy;	
}

double RtSPDBAR::get_packets_drop()
{
	return stat_pkts_drop;
}

double RtSPDBAR::get_pshf_created()
{
	return stat_pshf_created;
}
double RtSPDBAR::get_pstf_created()
{
	return stat_pstf_created;
}
double RtSPDBAR::get_swa_fail_msg_ratio()
{
	return stat_swa_fail_msg_ratio;	
}

double RtSPDBAR::get_swa_load()
{
	return stat_swa_load;
}

double RtSPDBAR::get_vca_fail_msg_ratio()
{
	return stat_vca_fail_msg_ratio;
}

double RtSPDBAR::get_vca_load()
{
	return stat_vca_load;
}
 
double RtSPDBAR::get_stat_packets()
{
	return stat_packets;	
}

double RtSPDBAR::get_stat_flits()
{
	return stat_flits;
}

double RtSPDBAR::get_stat_ib_cycles()
{
	return stat_ib_cycles;	
}

double RtSPDBAR::get_stat_rc_cycles()
{
	return stat_rc_cycles;
}

double RtSPDBAR::get_stat_vca_cycles()
{
	return stat_vca_cycles;
}
        
double RtSPDBAR::get_stat_swa_cycles()
{
	return stat_swa_cycles;
}
 
double RtSPDBAR::get_stat_st_cycles()
{
...
void
RtSPDBAR::handle_link_arrival_event ( IrisEvent* e )
{
    ...
    delete e; e=NULL;
    return ;
}		/* -----  end of function RtSPDBAR::handle_link_arrival_event  ----- */

void
RtSPDBAR::handle_reset_stat_event( IrisEvent* e)
{
    /* init the countes */
    stat_packets = 0;
    stat_flits = 0;
...
void
RtSPDBAR::do_switch_allocation_m() //Yi: m=modified
{
    bool was_swa_active = false; /* flag to make sure swa accesses are not double counted */
...
void
RtSPDBAR::do_virtual_channel_allocation()
{
/* flag to count accesses. In effect multiple w
 * inners can be chosen for one access of the arbiter */
bool was_vca_active = false;
...
void
RtSPDBAR::request_switch_allocation()
{
    for( uint i=0; i<(ports*vcs); i++)
...
void
RtSPDBAR::handle_tick_event ( IrisEvent* e )
{
    ticking = false;
    //do_switch_traversal();
    do_st();
...
}		/* -----  end of function RtSPDBAR::handle_input_arbitration_event  ----- */

string
RtSPDBAR::toString() const
{
    stringstream str;
    str << "RtSPDBAR"
        << "\t addr: " << address
...
void
RtSPDBAR::send_credit_back(uint i)
{
        ...
        if( input_buffer_state[i].pipe_stage == IB )
        {
        	...
        	if ( input_buffer_state[i].possible_oports.size() == 2 )
        	{
                ...
                for ( uint i=0; i<no_adaptive_aports; i++ )
                {
                    ...
                    if( static_cast<RtSPDBAR*>(static_cast<GenericLink*>(output_connections[porta])->output_connection)->neighbors[rc_port] != NULL )  // should check if exits
                    {
                    	a_aoc.push_back(rc_port);
                     temp=static_cast<RtSPDBAR*>(static_cast<GenericLink*>(output_connections[porta])->output_connection)->get_sum_bf(rc_port);
                ...
                for ( uint i=0; i<no_adaptive_bports; i++ )
                {
                    ...
                    if( static_cast<RtSPDBAR*>(static_cast<GenericLink*>(output_connections[portb])->output_connection)->neighbors[rc_port] != NULL )  // should check if exits
                    {
                        b_aoc.push_back(rc_port);
                        temp=static_cast<RtSPDBAR*>(static_cast<GenericLink*>(output_connections[portb])->output_connection)->get_sum_bf(rc_port);

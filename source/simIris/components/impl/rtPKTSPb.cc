...
#ifndef  _rtPKTSPb_cc_INC
#define  _rtPKTSPb_cc_INC
...
RtPKTSPb::RtPKTSPb ()
{
    name = "RtPKTSPb" ;
    ticking = false;
}  /* -----  end of method RtPKTSPb::RtPKTSPb  (constructor)  ----- */
...
void
RtPKTSPb::find_neighbors(uint p, uint v, uint cr, uint bs)
{
    ports =p;
    vcs =v;
    credits =cr;
    ...
    // for fault on neighbors, set up the pointer
    for(uint i=1; i<ports; i++)
    {
    	if (neighbors[i]!= -1)
        {
    		...
    		{
    			fon[i].push_back(&(static_cast<RtPKTSPb*>(static_cast<GenericLink*>(input_connections[i])->input_connection)->local_ft[j]));
    		}
        }
    	else
    	{
    ...
    // for congestion metric on neighbors, set up the pointer
        for(uint i=1; i<ports; i++)
        {
        	if (neighbors[i]!= -1)
            {
        		for(uint j=0; j<ports*2; j++)
        		{
        			cgmon[i].push_back(&(static_cast<RtPKTSPb*>(static_cast<GenericLink*>(input_connections[i])->input_connection)->local_cgm[j]));
        		}
            }
        	else
        	{
        		local_cgm[i] = 0;
        		for(uint j=0; j<ports*2; j++)
        			cgmon[i].push_back(NULL);
        	}
        }
    return ;
}
void
RtPKTSPb::set_fault(uint p)
{
...
void
RtPKTSPb::set_temp_fault(uint p,uint type)
{
...
void
RtPKTSPb::remove_fault(uint p,uint type)
{
	if(type==6000)
	{
...
void
RtPKTSPb::set_temp_opfault(uint p, double t, ullint d)
{
...
void
RtPKTSPb::set_temp_ipfault(uint p, double t, ullint d)
{
    IrisEvent* event = new IrisEvent();
    event->type = FAULT_INJECT_EVENT;
...
void
RtPKTSPb::handle_fault_remove_event(IrisEvent* e )
{
...
void
RtPKTSPb::handle_fault_inject_event(IrisEvent* e )
{
#ifdef _DEBUG_FI
	if(e->src_id==6000)
	{
...
void
RtPKTSPb::init (uint p, uint v, uint cr, uint bs)
{
    ports =p;
    vcs =v;
    credits =cr;
    ...
    local_cgm.resize(ports*2); // for 2 Virtual Network
    for(uint i=0; i<ports*2; i++)
    {
    	cgmon[0].push_back(&(local_cgm[i]));
    	//cgmon[0].push_back(&(local_cgm[i+ports])); // wrong , because of sequence access
    }

    if (cg_metric == BUFFER_SIZE )
    {
    	for(uint i=0; i<ports*2; i++)
    	{
    		local_cgm[i]=buffer_size*vcs/2; // Virtual Network is used
    	}
    }
    /* init the countes */
    stat_packets = 0;
...
void
RtPKTSPb::set_grid_x_location( uint port, uint x_node, uint value)
{
...
void
RtPKTSPb::set_grid_y_location( uint port, uint y_node, uint value)
{
...
void
RtPKTSPb::process_event ( IrisEvent* e )
{
    switch(e->type)
    {
        case LINK_ARRIVAL_EVENT:
#ifdef _DEBUG_ROUTER
        	_DBG_NOARG("; RtPKTSPb::LINK_ARRIVAL_EVENT ;");
#endif
            handle_link_arrival_event(e);
            break;
        case TICK_EVENT:
#ifdef _DEBUG_ROUTER
        	_DBG_NOARG("; RtPKTSPb::TICK_EVENT ;");
#endif
            handle_tick_event(e);
            break;
        case DETECT_DEADLOCK_EVENT:
#ifdef _DEBUG_ROUTER
        	_DBG_NOARG("; RtPKTSPb::DETECT_DEADLOCK_EVENT ;");
#endif
            handle_detect_deadlock_event(e);
            break;
        case FAULT_INJECT_EVENT:
#ifdef _DEBUG_ROUTER
        	_DBG_NOARG("; RtPKTSPb::FAULT_INJECT_EVENT ;");
#endif
            handle_fault_inject_event(e);
            break;
        case FAULT_REMOVE_EVENT:
#ifdef _DEBUG_ROUTER
        	_DBG_NOARG("; RtPKTSPb::FAULT_REMOVE_EVENT ;");
#endif
            handle_fault_remove_event(e);
            break;
        case RESET_STAT_EVENT:
#ifdef _DEBUG_ROUTER
        	_DBG_NOARG("; RtPKTSPb::RESET_STAT_EVENT ;");
#endif
        	handle_reset_stat_event(e);
            break;

        default:
            _DBG(";RtPKTSPb:: Unk event exception %d", e->type);
            break;
    }
    return ;
}		/* -----  end of function RtPKTSPb::process_event  ----- */

void 
RtPKTSPb::handle_detect_deadlock_event(IrisEvent* e )
{
    for ( uint i=0; i<ports; i++)
        for ( uint j=0; j<vcs; j++)
        {
...
double RtPKTSPb::get_average_packet_latency()
{
	return ((stat_total_packet_latency+0.0)/stat_packets);
}

double RtPKTSPb::get_last_flit_out_cycle()
{
	return last_flit_out_cycle;
}

double RtPKTSPb::get_flits_per_packet()
{
	return (stat_flits+0.0)/(stat_packets) ;
}

double RtPKTSPb::get_buffer_occupancy()
{
	return stat_buffer_occupancy;	
}

double RtPKTSPb::get_packets_drop()
{
	return stat_pkts_drop;
}

double RtPKTSPb::get_pshf_created()
{
	return stat_pshf_created;
}
double RtPKTSPb::get_pstf_created()
{
	return stat_pstf_created;
}
double RtPKTSPb::get_swa_fail_msg_ratio()
{
	return stat_swa_fail_msg_ratio;	
}

double RtPKTSPb::get_swa_load()
{
	return stat_swa_load;
}

double RtPKTSPb::get_vca_fail_msg_ratio()
{
	return stat_vca_fail_msg_ratio;
}

double RtPKTSPb::get_vca_load()
{
	return stat_vca_load;
}
 
double RtPKTSPb::get_stat_packets()
{
	return stat_packets;	
}

double RtPKTSPb::get_stat_flits()
{
	return stat_flits;
}

double RtPKTSPb::get_stat_ib_cycles()
{
	return stat_ib_cycles;	
}

double RtPKTSPb::get_stat_rc_cycles()
{
	return stat_rc_cycles;
}

double RtPKTSPb::get_stat_vca_cycles()
{
	return stat_vca_cycles;
}
        
double RtPKTSPb::get_stat_swa_cycles()
{
	return stat_swa_cycles;
}
 
double RtPKTSPb::get_stat_st_cycles()
{
...
void
RtPKTSPb::handle_link_arrival_event ( IrisEvent* e )
{
    ...
    delete e; e=NULL;
    return ;
}		/* -----  end of function RtPKTSPb::handle_link_arrival_event  ----- */

void
RtPKTSPb::handle_reset_stat_event( IrisEvent* e)
{
    /* init the countes */
    stat_packets = 0;
    stat_flits = 0;
...
void
RtPKTSPb::do_switch_allocation_m() //Yi: m=modified
{
    bool was_swa_active = false; /* flag to make sure swa accesses are not double counted */
...
void
RtPKTSPb::do_virtual_channel_allocation()
{
/* flag to count accesses. In effect multiple w
 * inners can be chosen for one access of the arbiter */
bool was_vca_active = false;
...
void
RtPKTSPb::request_switch_allocation()
{
    for( uint i=0; i<(ports*vcs); i++)
...
void
RtPKTSPb::handle_tick_event ( IrisEvent* e )
{
    ticking = false;
    //do_switch_traversal();
    do_st();
    ...
    for( uint i=0; i<(ports*vcs); i++)
    {
    	if( input_buffer_state[i].pipe_stage == IB )
    	{
			...
			uint op=-1;
			uint oc=-1;
			bool to_be_dropped = false;
    		// do_fon
    		// first dereference the pointer to get the no of fault on neighbor
    		...
    		else
    		{
    			if (cg_metric == BUFFER_SIZE )
    			{
    				...
    				if (input_buffer_state[i].vn == VN0)
    				{
    					...
    					{
    						local_cgm[p]=0;
    						for (uint ch=lowlimit;ch<highlimit;ch++)  // todo: virtual network
    							local_cgm[p]  +=  downstream_credits[p][ch];
    					}
    				...
    				if (input_buffer_state[i].vn == VN1)
    				{
    					lowlimit=vcs/2;
    					highlimit=vcs;
    					for (uint p=ports; p< 2*ports;p++)
    					{
    						local_cgm[p]=0;
    						for (uint ch=lowlimit;ch<highlimit;ch++)  // todo: virtual network
    							local_cgm[p]  +=  downstream_credits[p-ports][ch];
    					}
    			...
    			else if (cg_metric == FLIT_REMAIN )
    			{
    				...
    				for (uint p=1; p< ports;p++)
    				{
    					...
    					for( uint l=0; l<(ports*vcs); l++)
    					{
    						if(input_buffer_state[l].output_port == p)
    							local_cgm[p] += (input_buffer_state[l].remain_flits);
    					}
    				}
    				}
    				if(1)
    				...
    				if (input_buffer_state[i].vn == VN0)
    				{
    					for (uint p=1; p< ports;p++)
    					{
    						...
    						for( uint l=0; l<(ports*vcs); l++)
    						{
    							if((input_buffer_state[l].output_port == p)&&(input_buffer_state[l].vn==VN0))
    								local_cgm[p] += (input_buffer_state[l].remain_flits);
    						}
    					}
    				}
    				if (input_buffer_state[i].vn == VN1)
    				{
    					for (uint p=1; p< ports;p++)
    					{
    						...
    						for( uint l=0; l<(ports*vcs); l++)
    						{
    							if((input_buffer_state[l].output_port == p)&&(input_buffer_state[l].vn==VN1))
    								local_cgm[p] += (input_buffer_state[l].remain_flits);
    						}
    					}
    				}
    			...
    			// do_cgm  , based on 4 quadrants
    			vector<uint> cgm_quad;
    			cgm_quad.resize(4);
    			for (uint q=0; q<4; q++)
    			{
    				uint p1=-1, p2=-1;
    				...
    				case 3:
    					...
    					break;
    				default:
    					cout<<"wrong quadrant"<<endl;
    					break;
    				}
    				if (input_buffer_state[i].vn == VN0)
    				{
    					uint cgm_on1=local_cgm[p1], cgm_on2=local_cgm[p2];
    					if(cgmon[p1][p1]!=NULL)
    						cgm_on1 += (*(cgmon[p1][p1]))/2;
    					if(cgmon[p1][p2]!=NULL)
    						cgm_on1 += (*(cgmon[p1][p2]))/2;
    					if(cgmon[p2][p1]!=NULL)
    						cgm_on2 += (*(cgmon[p2][p1]))/2;
    					if(cgmon[p2][p2]!=NULL)
    						cgm_on2 += (*(cgmon[p2][p2]))/2;
    					if (cg_metric == BUFFER_SIZE )
    					{
    				...
    				if (input_buffer_state[i].vn == VN1)
    				{
    					uint cgm_on1=local_cgm[p1+ports], cgm_on2=local_cgm[p2+ports];
    					if(cgmon[p1][p1]!=NULL)
    						cgm_on1 += (*(cgmon[p1][p1+ports]))/2;
    					if(cgmon[p1][p2]!=NULL)
    						cgm_on1 += (*(cgmon[p1][p2+ports]))/2;
    					if(cgmon[p2][p1]!=NULL)
    						cgm_on2 += (*(cgmon[p2][p1+ports]))/2;
    					if(cgmon[p2][p2]!=NULL)
    						cgm_on2 += (*(cgmon[p2][p2+ports]))/2;
    					if (cg_metric == BUFFER_SIZE )
    					{
    			...
    			// analyze possible output port
    			vector <uint> minimal_possible_op;
    			vector <uint> nonminimal_possible_op;
    			for(uint j=0;j< input_buffer_state[i].possible_oports.size();j++)
    			{
    				if(input_buffer_state[i].is_misrouting.at(j))
    				{
    					nonminimal_possible_op.push_back(input_buffer_state[i].possible_oports.at(j));
    				}
    				else
    					minimal_possible_op.push_back(input_buffer_state[i].possible_oports.at(j));
    			}
    			uint nb_mpop=minimal_possible_op.size();
    			uint nb_nonmpop=nonminimal_possible_op.size();

    			if (nb_mpop ==2 )
    			{
    				switch (minimal_possible_op[0])
    				{
    				case 1:
    					if(minimal_possible_op[1]== 3)
    						op = cgm_quad[2];
    					else if (minimal_possible_op[1]== 4)
    						op = cgm_quad[3];
    					break;
    				case 2:
    					if(minimal_possible_op[1]== 3)
    					...
    					break;
    				case 3:
    					if(minimal_possible_op[1]== 1)
    						op = cgm_quad[2];
    					else if (minimal_possible_op[1]== 2)
    						op = cgm_quad[1];
    					break;
    				case 4:
    					if(minimal_possible_op[1]== 1)
    			...
    			{
    				op = minimal_possible_op[0];
    			}
    			else if (nb_mpop == 0)
    			{
    				if (nb_nonmpop >=2 ) // only check local cgm
    				{
    					for(uint i=0;i<nb_nonmpop;i++)
    					{
    						uint temp_op=nonminimal_possible_op[i];
    						metrics.push_back(pair<uint,uint>(temp_op,local_cgm[temp_op]));
    					}
    					if (cg_metric == BUFFER_SIZE )
    					{
    				...
    				else if (nb_nonmpop == 1)
    				{
    					op = nonminimal_possible_op[0];
    				}
    				else if (nb_nonmpop == 0)
    				{
    					to_be_dropped = true;
    				}
    			}
    		...
    		if(to_be_dropped) //else  // drop pkt
    		{
...
}		/* -----  end of function RtPKTSPb::handle_input_arbitration_event  ----- */

string
RtPKTSPb::toString() const
{
    stringstream str;
    str << "RtPKTSPb"
        << "\t addr: " << address
...
void
RtPKTSPb::send_credit_back(uint i)
{
        ...
        if( input_buffer_state[i].pipe_stage == IB )
        {
        	...
        	if ( input_buffer_state[i].possible_oports.size() == 2 )
        	{
                ...
                for ( uint i=0; i<no_adaptive_aports; i++ )
                {
                    ...
                    if( static_cast<RtPKTSPb*>(static_cast<GenericLink*>(output_connections[porta])->output_connection)->neighbors[rc_port] != NULL )  // should check if exits
                    {
                    	a_aoc.push_back(rc_port);
                     temp=static_cast<RtPKTSPb*>(static_cast<GenericLink*>(output_connections[porta])->output_connection)->get_sum_bf(rc_port);
                ...
                for ( uint i=0; i<no_adaptive_bports; i++ )
                {
                    ...
                    if( static_cast<RtPKTSPb*>(static_cast<GenericLink*>(output_connections[portb])->output_connection)->neighbors[rc_port] != NULL )  // should check if exits
                    {
                        b_aoc.push_back(rc_port);
                        temp=static_cast<RtPKTSPb*>(static_cast<GenericLink*>(output_connections[portb])->output_connection)->get_sum_bf(rc_port);

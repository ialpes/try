...
#ifndef  _rtPRDBAR_cc_INC
#define  _rtPRDBAR_cc_INC
...
RtPRDBAR::RtPRDBAR ()
{
    name = "RtPRDBAR" ;
    ticking = false;
}  /* -----  end of method RtPRDBAR::RtPRDBAR  (constructor)  ----- */
...
void
RtPRDBAR::find_neighbors(uint p, uint v, uint cr, uint bs)
{
    ports =p;
    vcs =v;
    credits =cr;
...
void
RtPRDBAR::set_temp_fault(uint p,uint type)
{
...
void
RtPRDBAR::remove_fault(uint p,uint type)
{
	if(type==6000)
...
void
RtPRDBAR::set_temp_opfault(uint p, double t, ullint d)
{
...
void
RtPRDBAR::set_temp_ipfault(uint p, double t, ullint d)
{
    IrisEvent* event = new IrisEvent();
    event->type = FAULT_INJECT_EVENT;
...
void
RtPRDBAR::handle_fault_remove_event(IrisEvent* e )
{
...
void
RtPRDBAR::handle_fault_inject_event(IrisEvent* e )
{
#ifdef _DEBUG_FI
	if(e->src_id==6000)
	{
...
void
RtPRDBAR::init (uint p, uint v, uint cr, uint bs)
{
    ports =p;
    vcs =v;
    credits =cr;
...
void
RtPRDBAR::set_grid_x_location( uint port, uint x_node, uint value)
{
...
void
RtPRDBAR::set_grid_y_location( uint port, uint y_node, uint value)
{
...
void
RtPRDBAR::process_event ( IrisEvent* e )
{
    switch(e->type)
    {
        case LINK_ARRIVAL_EVENT:
#ifdef _DEBUG_ROUTER
        	_DBG_NOARG("; RtPRDBAR::LINK_ARRIVAL_EVENT ;");
#endif
            handle_link_arrival_event(e);
            break;
        case TICK_EVENT:
#ifdef _DEBUG_ROUTER
        	_DBG_NOARG("; RtPRDBAR::TICK_EVENT ;");
#endif
            handle_tick_event(e);
            break;
        case CHECK_EVENT:
#ifdef _DEBUG_ROUTER
        	_DBG_NOARG("; RtPRDBAR::CHECK_EVENT ;");
#endif
        	handle_check_event(e);
        	break;
        case DETECT_DEADLOCK_EVENT:
#ifdef _DEBUG_ROUTER
        	_DBG_NOARG("; RtPRDBAR::DETECT_DEADLOCK_EVENT ;");
#endif
            handle_detect_deadlock_event(e);
            break;
        case FAULT_INJECT_EVENT:
#ifdef _DEBUG_ROUTER
        	_DBG_NOARG("; RtPRDBAR::FAULT_INJECT_EVENT ;");
#endif
            handle_fault_inject_event(e);
            break;
        case FAULT_REMOVE_EVENT:
#ifdef _DEBUG_ROUTER
        	_DBG_NOARG("; RtPRDBAR::FAULT_REMOVE_EVENT ;");
#endif
            handle_fault_remove_event(e);
            break;
        case RESET_STAT_EVENT:
#ifdef _DEBUG_ROUTER
        	_DBG_NOARG("; RtPRDBAR::RESET_STAT_EVENT ;");
#endif
        	handle_reset_stat_event(e);
            break;

        default:
            _DBG(";RtPRDBAR:: Unk event exception %d", e->type);
            break;
    }
    return ;
}		/* -----  end of function RtPRDBAR::process_event  ----- */
void
RtPRDBAR::handle_check_event (IrisEvent* e)
{
...
void 
RtPRDBAR::handle_detect_deadlock_event(IrisEvent* e )
{
    for ( uint i=0; i<ports; i++)
        for ( uint j=0; j<vcs; j++)
        {
...
double RtPRDBAR::get_average_packet_latency()
{
	return ((stat_total_packet_latency+0.0)/stat_packets);
}

double RtPRDBAR::get_last_flit_out_cycle()
{
	return last_flit_out_cycle;
}

double RtPRDBAR::get_flits_per_packet()
{
	return (stat_flits+0.0)/(stat_packets) ;
}

double RtPRDBAR::get_buffer_occupancy()
{
	return stat_buffer_occupancy;	
}

double RtPRDBAR::get_packets_drop()
{
	return stat_pkts_drop;
}

double RtPRDBAR::get_pshf_created()
{
	return stat_pshf_created;
}
double RtPRDBAR::get_pstf_created()
{
	return stat_pstf_created;
}
double RtPRDBAR::get_swa_fail_msg_ratio()
{
	return stat_swa_fail_msg_ratio;	
}

double RtPRDBAR::get_swa_load()
{
	return stat_swa_load;
}

double RtPRDBAR::get_vca_fail_msg_ratio()
{
	return stat_vca_fail_msg_ratio;
}

double RtPRDBAR::get_vca_load()
{
	return stat_vca_load;
}
 
double RtPRDBAR::get_stat_packets()
{
	return stat_packets;	
}

double RtPRDBAR::get_stat_flits()
{
	return stat_flits;
}

double RtPRDBAR::get_stat_ib_cycles()
{
	return stat_ib_cycles;	
}

double RtPRDBAR::get_stat_rc_cycles()
{
	return stat_rc_cycles;
}

double RtPRDBAR::get_stat_vca_cycles()
{
	return stat_vca_cycles;
}
        
double RtPRDBAR::get_stat_swa_cycles()
{
	return stat_swa_cycles;
}
 
double RtPRDBAR::get_stat_st_cycles()
{
...
void
RtPRDBAR::handle_link_arrival_event ( IrisEvent* e )
{
    ...
    delete e; e=NULL;
    return ;
}		/* -----  end of function RtPRDBAR::handle_link_arrival_event  ----- */

void
RtPRDBAR::handle_reset_stat_event( IrisEvent* e)
{
    /* init the countes */
    stat_packets = 0;
    stat_flits = 0;
...
void
RtPRDBAR::do_switch_allocation_m() //Yi: m=modified
{
    bool was_swa_active = false; /* flag to make sure swa accesses are not double counted */
    ...
    for ( uint i=0; i<input_buffer_state.size(); ++i)
        if( input_buffer_state.at(i).pipe_stage==SW_TRAVERSAL )
        {
            ...
            if ( downstream_credits.at(op).at(oc) )
            {
                ...
                if(f->type != HEAD)
                {
                	if(input_buffer_state[i].sent_head)
                	{
                		if(input_buffer_state[i].is_pseudo_tf_created)
                			is_dropped=true;
                	}
                	else is_dropped=true;
                }
                	...
                	if ((!found_opfault) && (!is_dropped))
                	{
                		if(f->type == TAIL && f->pseudo && f->valid)
                		{
    						...
    						IrisEvent* event = new IrisEvent();
    												event->type = CHECK_EVENT;
    												event->time = 2000;
    												Simulator::Schedule( Simulator::Now()+2000,
    														&NetworkComponent::process_event, this, event);
                ...
                else if((found_opfault)||(is_dropped))// output port fault
                {
#define _DROPPKT
#ifdef _DROPPKT
                	bool sendback=true;
...
#ifdef _DEBUG_FI
                _DBG("; ST ; Fail ; ip_%d*vcs_%d+ic_%d(=%d) ft=%d",ip,vcs,ic,ip*vcs+ic, f->type);
        if(f->type ==HEAD)
        {
...
void
RtPRDBAR::do_virtual_channel_allocation()
{
/* flag to count accesses. In effect multiple w
 * inners can be chosen for one access of the arbiter */
bool was_vca_active = false;
...
void
RtPRDBAR::request_switch_allocation()
{
    for( uint i=0; i<(ports*vcs); i++)
...
void
RtPRDBAR::handle_tick_event ( IrisEvent* e )
{
    ticking = false;
    //do_switch_traversal();
    do_st();
            		...
            		else if (cg_metric == BUFFER_SIZE )
            		{
            			...
            			for(uint j=0;j< input_buffer_state[i].possible_oports.size();j++)
            			{
            				...
            				if(!input_buffer_state[i].is_misrouting.at(j))
            				{
            					...
            					{
            						temp_cgm += (downstream_credits[input_buffer_state[i].possible_oports.at(j)][k]);
            					}
								...
								if(input_buffer_state[i].is_misrouting.at(j))
								{
									...
									if(temp_op!=0)
									{
										...
										case 1:// op is west , check the in_buffer of east port
											p=2;
											index=1+metric_index[p];
											size=dest_x-current_x; // is misrouting
											//size=current_x-dest_x;
											for(uint t=0;t<size;t++)
											{
												temp_cg_metric +=metric_x[index+t]<<(size-1-t);
										...
										case 2:// op is east , check the in_buffer of west port
											p=1;
											index=1+metric_index[p];
											size=current_x-dest_x;// is misrouting
											//size=dest_x-current_x;
											for(uint t=0;t<size;t++)
											{
												//uint a=metric_x[index+t];
...
}		/* -----  end of function RtPRDBAR::handle_input_arbitration_event  ----- */

string
RtPRDBAR::toString() const
{
    stringstream str;
    str << "RtPRDBAR"
        << "\t addr: " << address
...
void
RtPRDBAR::send_credit_back(uint i)
{
        ...
        if( input_buffer_state[i].pipe_stage == IB )
        {
        	...
        	if ( input_buffer_state[i].possible_oports.size() == 2 )
        	{
                ...
                for ( uint i=0; i<no_adaptive_aports; i++ )
                {
                    ...
                    if( static_cast<RtPRDBAR*>(static_cast<GenericLink*>(output_connections[porta])->output_connection)->neighbors[rc_port] != NULL )  // should check if exits
                    {
                    	a_aoc.push_back(rc_port);
                     temp=static_cast<RtPRDBAR*>(static_cast<GenericLink*>(output_connections[porta])->output_connection)->get_sum_bf(rc_port);
                ...
                for ( uint i=0; i<no_adaptive_bports; i++ )
                {
                    ...
                    if( static_cast<RtPRDBAR*>(static_cast<GenericLink*>(output_connections[portb])->output_connection)->neighbors[rc_port] != NULL )  // should check if exits
                    {
                        b_aoc.push_back(rc_port);
                        temp=static_cast<RtPRDBAR*>(static_cast<GenericLink*>(output_connections[portb])->output_connection)->get_sum_bf(rc_port);

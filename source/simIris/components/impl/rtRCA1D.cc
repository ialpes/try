...
#ifndef  _rtRCA1D_cc_INC
#define  _rtRCA1D_cc_INC
...
RtRCA1D::RtRCA1D ()
{
	name = "RtRCA1D" ;
	ticking = false;
}  /* -----  end of method RtRCA1D::RtRCA1D  (constructor)  ----- */
...
void
RtRCA1D::find_neighbors(uint p, uint v, uint cr, uint bs)
{
	ports =p;
	vcs =v;
	credits =cr;
...
void
RtRCA1D::set_temp_opfault(uint p, double t, ullint d)
{
...
void
RtRCA1D::set_temp_ipfault(uint p, double t, ullint d)
{
	return ;
}

void
RtRCA1D::handle_fault_inject_event(IrisEvent* e )
{
...
void
RtRCA1D::init (uint p, uint v, uint cr, uint bs)
{
	ports =p;
	vcs =v;
	credits =cr;
...
void
RtRCA1D::set_grid_x_location( uint port, uint x_node, uint value)
{
...
void
RtRCA1D::set_grid_y_location( uint port, uint y_node, uint value)
{
...
void
RtRCA1D::process_event ( IrisEvent* e )
{
	switch(e->type)
	{
	case LINK_ARRIVAL_EVENT:
#ifdef _DEBUG_ROUTER
		_DBG("; RtRCA1D::LINK_ARRIVAL_EVENT ; time:%f", Simulator::Now());
#endif
		handle_link_arrival_event(e);
		break;
	case TICK_EVENT:
#ifdef _DEBUG_ROUTER
		_DBG("; RtRCA1D::TICK_EVENT ; time:%f", Simulator::Now());
#endif
		handle_tick_event(e);
		break;
...
#ifdef _DEBUG_ROUTER
		_DBG("; RtRCA1D::FAULT_INJECT_EVENT ; time:%f", Simulator::Now());
#endif
		handle_fault_inject_event(e);
		break;
	case FAULT_REMOVE_EVENT:
#ifdef _DEBUG_ROUTER
		_DBG("; RtRCA1D::FAULT_REMOVE_EVENT ; time:%f", Simulator::Now());
#endif
		handle_fault_remove_event(e);
		break;
	case RESET_STAT_EVENT:
#ifdef _DEBUG_ROUTER
		_DBG("; RtRCA1D::RESET_STAT_EVENT time:%f", Simulator::Now());
#endif
		handle_reset_stat_event(e);
		break;
	case DBAR_ARRIVAL_EVENT:
#ifdef _DEBUG_ROUTER
		_DBG("; RtRCA1D::DBAR_ARRIVAL_EVENT time:%f", Simulator::Now());
#endif
		handle_dbar_arrival_event(e);
		break;

	default:
		_DBG(";RtRCA1D:: Unk event exception %d", e->type);
		break;
	}
	return ;
}		/* -----  end of function RtRCA1D::process_event  ----- */
void 
RtRCA1D::handle_dbar_arrival_event(IrisEvent* e )
{
...
void
RtRCA1D::handle_detect_deadlock_event(IrisEvent* e )
{
	for ( uint i=0; i<ports; i++)
		for ( uint j=0; j<vcs; j++)
		{
...
double RtRCA1D::get_average_packet_latency()
{
	return ((stat_total_packet_latency+0.0)/stat_packets);
}

double RtRCA1D::get_last_flit_out_cycle()
{
	return last_flit_out_cycle;
}

double RtRCA1D::get_flits_per_packet()
{
	return (stat_flits+0.0)/(stat_packets) ;
}

double RtRCA1D::get_buffer_occupancy()
{
	return stat_buffer_occupancy;	
}

double RtRCA1D::get_packets_drop()
{
	return stat_pkts_drop;
}

double RtRCA1D::get_swa_fail_msg_ratio()
{
	return stat_swa_fail_msg_ratio;	
}

double RtRCA1D::get_swa_load()
{
	return stat_swa_load;
}

double RtRCA1D::get_vca_fail_msg_ratio()
{
	return stat_vca_fail_msg_ratio;
}

double RtRCA1D::get_vca_load()
{
	return stat_vca_load;
}

double RtRCA1D::get_stat_packets()
{
	return stat_packets;	
}

double RtRCA1D::get_stat_flits()
{
	return stat_flits;
}

double RtRCA1D::get_stat_ib_cycles()
{
	return stat_ib_cycles;	
}

double RtRCA1D::get_stat_rc_cycles()
{
	return stat_rc_cycles;
}

double RtRCA1D::get_stat_vca_cycles()
{
	return stat_vca_cycles;
}

double RtRCA1D::get_stat_swa_cycles()
{
	return stat_swa_cycles;
}

double RtRCA1D::get_stat_st_cycles()
{
...
void
RtRCA1D::handle_link_arrival_event ( IrisEvent* e )
{
	LinkArrivalData* data = static_cast<LinkArrivalData*>(e->event_data.at(0));
	...
	else if(data->type == METRIC_ID)
	{
		...
		nlcvr[port]=temp[0];
#ifdef  _DEBUG_RCA
			_DBG("; link_arrival ; metric; port %d; nlcvr %d;", port, nlcvr[port]);
#endif
	...
	delete e;
	return ;
}		/* -----  end of function RtRCA1D::handle_link_arrival_event  ----- */

void
RtRCA1D::handle_reset_stat_event( IrisEvent* e)
{
	/* init the countes */
	stat_packets = 0;
	stat_flits = 0;
...
RtRCA1D::do_rca()
{
	...
	for(uint p=1;p<ports;p++)
	{
		...
		if(neighbors[op]== -1)
		{
			to_propagate[op]=false;
		}

		if(to_propagate[op]==true)
		{
			vector<uint> * mtc = new vector<uint>;
			uint size = 1;
			pvr[p] = (cvr[p] + nlcvr[p])/2;
			mtc->push_back(pvr[p]);
			...
			data->p = mtc;
			IrisEvent* event = new IrisEvent();
			event->type = LINK_ARRIVAL_EVENT; //TODO: merge to LINK_ARRIVAL_EVENT DBAR_ARRIVAL_EVENT
...
#ifdef  _DEBUG_RCA
			_DBG("; do_rca(); oport %d; port %d; cvr %d; nlcvr %d; pvr %d;", op, p, cvr[p],nlcvr[p], pvr[p] );
#endif
		}
	}
...
void
RtRCA1D::do_switch_allocation_m() //Yi: m=modified
{
	bool was_swa_active = false; /* flag to make sure swa accesses are not double counted */
...
void
RtRCA1D::do_virtual_channel_allocation()
{
	/* flag to count accesses. In effect multiple w
	 * inners can be chosen for one access of the arbiter */
	bool was_vca_active = false;
...
void
RtRCA1D::request_switch_allocation()
{
	for( uint i=0; i<(ports*vcs); i++)
...
void
RtRCA1D::handle_tick_event ( IrisEvent* e )
{
	ticking = false;
	//do_switch_traversal();
	do_st();
					...
					else if (cg_metric == BUFFER_SIZE )
					{
						if(input_buffer_state[i].possible_oports.size() >1)
						{
							for(uint j=0;j< input_buffer_state[i].possible_oports.size();j++)
							{
								...
								{
									metrics.push_back(pair<uint,uint>(temp_op, pvr[temp_op]));
								}
...
}		/* -----  end of function RtRCA1D::handle_input_arbitration_event  ----- */

string
RtRCA1D::toString() const
{
	stringstream str;
	str << "RtRCA1D"
			<< "\t addr: " << address
...
void
RtRCA1D::send_credit_back(uint i)
{
		...
		if( input_buffer_state[i].pipe_stage == IB )
		{
			...
			if ( input_buffer_state[i].possible_oports.size() == 2 )
			{
				...
				for ( uint i=0; i<no_adaptive_aports; i++ )
				{
					...
					if( static_cast<RtRCA1D*>(static_cast<GenericLink*>(output_connections[porta])->output_connection)->neighbors[rc_port] != NULL )  // should check if exits
					{
						a_aoc.push_back(rc_port);
						temp=static_cast<RtRCA1D*>(static_cast<GenericLink*>(output_connections[porta])->output_connection)->get_sum_bf(rc_port);
				...
				for ( uint i=0; i<no_adaptive_bports; i++ )
				{
					...
					if( static_cast<RtRCA1D*>(static_cast<GenericLink*>(output_connections[portb])->output_connection)->neighbors[rc_port] != NULL )  // should check if exits
					{
						b_aoc.push_back(rc_port);
						temp=static_cast<RtRCA1D*>(static_cast<GenericLink*>(output_connections[portb])->output_connection)->get_sum_bf(rc_port);

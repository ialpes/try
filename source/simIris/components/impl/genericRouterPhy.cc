...
bool
GenericRouterPhy::check_neighbours(uint port) const
{
	return neighbours[port];
}
void
GenericRouterPhy::find_neighbors(uint p, uint v, uint cr, uint bs)
{
    ports =p;
    vcs =v;
    credits =cr;
    ...
    //nop_rc.resize(ports);//neighbors.size());  // either index with port, or with size of neighbors
...
GenericRouterPhy::process_event ( IrisEvent* e )
{
    switch(e->type)
    {
...
#ifdef _DEBUG_ROUTER
            _DBG("; GenericRouterPhy::LINK_ARRIVAL_EVENT ; time:%f", Simulator::Now());
#endif
        ...
        case TICK_EVENT:
             ...
             * in backward order */
#ifdef _DEBUG_ROUTER
            _DBG("; GenericRouterPhy::TICK_EVENT ; time:%f", Simulator::Now());
#endif
            handle_tick_event(e);
            break;
        default:
...
GenericRouterPhy::handle_link_arrival_event ( IrisEvent* e )
{
     ...
     *  switch traversal stage in this example or whatever is the last
...
//            stat_ib_cycles++;
    ...
    else if ( data->type == CREDIT_ID)
    {
         ...
         * unique addresses.
         */
        _DBG(" Got a credit port:%d vc:%d in_buffers[port][vc]:%d(downstream_credits[%d][%d]=%d) ", port, data->vc, downstream_credits[port][data->vc], port, data->vc, downstream_credits[port][data->vc]);
    ...
    for( uint i=0; i<ports*vcs; i++)
        if( input_buffer_state[i].pipe_stage == SW_TRAVERSAL)
        {
            ...
            if( !xbar.is_empty(oport,0) 
                ...
                && in_buffers[iport].buffers[0].size()>0)
            {
...
#ifdef _DEEP_DEBUG
                _DBG(" ST; downstream_credits[%d][0]=%d ",oport, downstream_credits[oport][0]);
#endif
        ...
        if( input_buffer_state[i].pipe_stage == FULL || input_buffer_state[i].pipe_stage == EMPTY )
        {
            if( in_buffers[inport].get_occupancy(invc) > 0)
            {
                ...
                if( f->type == HEAD )
                {
                    ...
                    input_buffer_state[inport*vcs+f->vc].possible_ovcs.resize(1);
                    input_buffer_state[inport*vcs+f->vc].possible_oports.resize(1);

                    input_buffer_state[inport*vcs+f->vc].possible_oports[0] = decoders[inport].get_output_port(f->vc);
                    input_buffer_state[inport*vcs+f->vc].possible_ovcs[0] = 0;
                    ...
                    input_buffer_state[inport*vcs+f->vc].arrival_time= ceil(Simulator::Now());
                    input_buffer_state[inport*vcs+f->vc].clear_message= false;
                    //input_buffer_state[inport*vcs+f->vc].flits_in_ib = 0;
...
GenericRouterPhy::handle_tick_event ( IrisEvent* e )
{
    ...
    for( uint i=0; i<ports; i++)
    {
      cout << "downstream_credits["<<i<<"]="<<downstream_credits[i][0]<<endl;
    }
#endif

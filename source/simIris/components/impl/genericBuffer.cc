...
{
//TODO only push_back into buffer when it isn't full !!!
	buffers[push_channel].push_back(f);

#ifdef _DEBUG
    if (buffers[push_channel].size() > buffer_size) {
	//_DBG("; GenericBuffer::buffers[%d].size()=%d ; time:%f", push_channel, buffers[push_channel].size(),Simulator::Now());
            cerr << " buffers["<<push_channel<<"].size()= "<<buffers[push_channel].size()<< " time: "<< Simulator::Now() <<endl;
    }
#endif
...
void
GenericBuffer::push_front ( Flit* f )
{
//TODO only push_back into buffer when it isn't full !!!
	buffers[push_channel].push_front(f);
...
uint
GenericBuffer::get_no_empty_vc() const
{
	uint count=0;
	for(uint i=0;i<vcs;i++)
	{

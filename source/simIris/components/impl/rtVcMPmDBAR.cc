...
#ifndef  _rtVcMPmDBAR_cc_INC
#define  _rtVcMPmDBAR_cc_INC

#include	"rtVcMPmDBAR.h"
using namespace std;
...
RtVcMPmDBAR::RtVcMPmDBAR ()
{
	name = "RtVcMPmDBAR" ;
	ticking = false;
}  /* -----  end of method RtVcMPmDBAR::RtVcMPmDBAR  (constructor)  ----- */
...
void
RtVcMPmDBAR::find_neighbors(uint p, uint v, uint cr, uint bs)
{
	ports =p;
	vcs =v;
	credits =cr;
...
void
RtVcMPmDBAR::remove_fault(uint p)
{
	fault_ports.erase(remove(fault_ports.begin(),fault_ports.end(),p),fault_ports.end());
	return ;
}
void
RtVcMPmDBAR::handle_fault_remove_event(IrisEvent* e )
{
...
void
RtVcMPmDBAR::set_temp_opfault(uint p, double t, ullint d)
{
...
void
RtVcMPmDBAR::set_temp_ipfault(uint p, double t, ullint d)
{
	return ;
}

void
RtVcMPmDBAR::handle_fault_inject_event(IrisEvent* e )
{
...
void
RtVcMPmDBAR::init (uint p, uint v, uint cr, uint bs)
{
	ports =p;
	vcs =v;
	credits =cr;
	...
	metric_x.resize(grid_size+1);
	metric_y.resize(grid_size+1);
	to_propagate.resize(ports);
	metric_index.resize(ports);
	...
	for(uint i=0;i<ports;i++)
	{
		...
		case 0:
			...
			break;
		case 1: // west
			metric_index[i]=node_ip%grid_size+1;
			break;
		case 2: // east
			metric_index[i]=0;
			break;
		...
		default:
		{
			printf("Error: no this port");
			exit(-1);
		}
		}
#ifdef  _DEBUG_DBAR
		_DBG("; port_%d index_%d ",i, metric_index[i]);
#endif
	}
...
void
RtVcMPmDBAR::set_grid_x_location( uint port, uint x_node, uint value)
{
...
void
RtVcMPmDBAR::set_grid_y_location( uint port, uint y_node, uint value)
{
...
void
RtVcMPmDBAR::process_event ( IrisEvent* e )
{
	switch(e->type)
	{
	case LINK_ARRIVAL_EVENT:
#ifdef _DEBUG_ROUTER
		_DBG("; RtVcMPmDBAR::LINK_ARRIVAL_EVENT ; time:%f", Simulator::Now());
#endif
		handle_link_arrival_event(e);
		break;
	case TICK_EVENT:
#ifdef _DEBUG_ROUTER
		_DBG("; RtVcMPmDBAR::TICK_EVENT ; time:%f", Simulator::Now());
#endif
		handle_tick_event(e);
		break;
...
#ifdef _DEBUG_ROUTER
		_DBG("; RtVcMPmDBAR::FAULT_INJECT_EVENT ; time:%f", Simulator::Now());
#endif
		handle_fault_inject_event(e);
		break;
	case FAULT_REMOVE_EVENT:
#ifdef _DEBUG_ROUTER
		_DBG("; RtVcMPmDBAR::FAULT_REMOVE_EVENT ; time:%f", Simulator::Now());
#endif
		handle_fault_remove_event(e);
		break;
	case RESET_STAT_EVENT:
#ifdef _DEBUG_ROUTER
		_DBG("; RtVcMPmDBAR::RESET_STAT_EVENT time:%f", Simulator::Now());
#endif
		handle_reset_stat_event(e);
		break;
	case DBAR_ARRIVAL_EVENT:
#ifdef _DEBUG_ROUTER
		_DBG("; RtVcMPmDBAR::DBAR_ARRIVAL_EVENT time:%f", Simulator::Now());
#endif
		handle_dbar_arrival_event(e);
		break;

	default:
		_DBG(";RtVcMPmDBAR:: Unk event exception %d", e->type);
		break;
	}
	return ;
}		/* -----  end of function RtVcMPmDBAR::process_event  ----- */
void 
RtVcMPmDBAR::handle_dbar_arrival_event(IrisEvent* e )
{
	LinkArrivalData* data = static_cast<LinkArrivalData*>(e->event_data.at(0));
	if(data->type == METRIC_ID)
	{
		...
		uint size=data->vc;
		temp.resize(size);
		temp=*(static_cast<vector<uint> * >(data->p));
		uint pt=-1;
		switch(port)
		{
		case 1:
			pt=2; // port of update direction
			for(uint i=0;i<size;i++)
			{
				metric_x[metric_index[pt]+1+i]=temp[i];
			}
			to_propagate[pt]=true;
			break;
		case 2:
			pt=1; // port of update direction
			for(uint i=0;i<size;i++)
			{
				metric_x[metric_index[pt]+1+i]=temp[i];
			}
			...
			break;
		case 3:
			pt=4; // port of update direction
			for(uint i=0;i<size;i++)
			{
				metric_y[metric_index[pt]+1+i]=temp[i];
			}
			to_propagate[pt]=true;
			break;
		case 4:
			pt=3; // port of update direction
			for(uint i=0;i<size;i++)
			{
				metric_y[metric_index[pt]+1+i]=temp[i];
			}
			...
			break;
		default:
			printf("\n handle_dbar_arrival_event() wrong port for dbar \n");
			exit(-1);
		}

		delete static_cast<vector<uint> * >(data->p);
		delete e;
		...
		//string s=str.str();
		//DBG("; handle_dbar_arrival_event inport_%d pt_%d metric %s", port, pt,s);
		//_DBG("; handle_dbar_arrival_event inport_%d pt_%d metric %s", port, pt,str.str());
		//		cerr<< str.str() <<endl;
#endif
#ifdef  _DEBUG_DBAR
		ostringstream str_x,str_y;
		for(uint i=0;i<metric_x.size();i++)
		{
			str_x << " "<< metric_x[i]<< " ";
		}
		for(uint i=0;i<metric_y.size();i++)
		{
			str_y << " "<< metric_y[i]<< " ";
		}
		//char c[]=str.str();
		_DBG("; handle_dbar_arrival_event; %s", "metric_x");
		cerr << str_x.str();
		_DBG("; handle_dbar_arrival_event; %s", "metric_y");
		cerr << str_y.str();
		cerr <<endl;
#endif
	}
...
void
RtVcMPmDBAR::handle_detect_deadlock_event(IrisEvent* e )
{
	for ( uint i=0; i<ports; i++)
		for ( uint j=0; j<vcs; j++)
		{
...
double RtVcMPmDBAR::get_average_packet_latency()
{
	return ((stat_total_packet_latency+0.0)/stat_packets);
}

double RtVcMPmDBAR::get_last_flit_out_cycle()
{
	return last_flit_out_cycle;
}

double RtVcMPmDBAR::get_flits_per_packet()
{
	return (stat_flits+0.0)/(stat_packets) ;
}

double RtVcMPmDBAR::get_buffer_occupancy()
{
	return stat_buffer_occupancy;	
}

double RtVcMPmDBAR::get_packets_drop()
{
	return stat_pkts_drop;
}

double RtVcMPmDBAR::get_swa_fail_msg_ratio()
{
	return stat_swa_fail_msg_ratio;	
}

double RtVcMPmDBAR::get_swa_load()
{
	return stat_swa_load;
}

double RtVcMPmDBAR::get_vca_fail_msg_ratio()
{
	return stat_vca_fail_msg_ratio;
}

double RtVcMPmDBAR::get_vca_load()
{
	return stat_vca_load;
}

double RtVcMPmDBAR::get_stat_packets()
{
	return stat_packets;	
}

double RtVcMPmDBAR::get_stat_flits()
{
	return stat_flits;
}

double RtVcMPmDBAR::get_stat_ib_cycles()
{
	return stat_ib_cycles;	
}

double RtVcMPmDBAR::get_stat_rc_cycles()
{
	return stat_rc_cycles;
}

double RtVcMPmDBAR::get_stat_vca_cycles()
{
	return stat_vca_cycles;
}

double RtVcMPmDBAR::get_stat_swa_cycles()
{
	return stat_swa_cycles;
}

double RtVcMPmDBAR::get_stat_st_cycles()
{
...
void
RtVcMPmDBAR::handle_link_arrival_event ( IrisEvent* e )
{
	LinkArrivalData* data = static_cast<LinkArrivalData*>(e->event_data.at(0));
	...
	delete e;
	return ;
}		/* -----  end of function RtVcMPmDBAR::handle_link_arrival_event  ----- */

void
RtVcMPmDBAR::handle_reset_stat_event( IrisEvent* e)
{
	/* init the countes */
	stat_packets = 0;
	stat_flits = 0;
...
RtVcMPmDBAR::do_dbar()
{
	uint current_x=node_ip%grid_size;
	uint current_y=node_ip/grid_size;
	...
	for(uint p=1;p<ports;p++)
	{
		...
		if(in_buffers[p].get_no_empty_vc()<4)
			local_metric[p]=1;
		else
			local_metric[p]=0;
		...
		if(p<=2)
		{
			if(metric_x[metric_index[p]]!=local_metric[p])
			{
				metric_x[metric_index[p]]=local_metric[p];
				to_propagate[p]=true;
			}
		}
		else
		{
			if(metric_y[metric_index[p]]!=local_metric[p])
			{
				metric_y[metric_index[p]]=local_metric[p];
				to_propagate[p]=true;
			}
		}
		...
		if(neighbors[p]== -1)
		{
			to_propagate[p]=false;
		}

		if(to_propagate[p]==true)
		{
			uint op=-1;
			uint size=0;
			vector<uint> * mtc = new vector<uint>;
			uint t=0;
			switch(p)
			{
			case 1: // west
				op=1;
				size=grid_size+1-metric_index[p];
				assert (size>0);
				for(uint i=0;i<size;i++)
				{
					mtc->push_back(metric_x[metric_index[p]+i]);
				}
				break;
			case 2: // east
				op=2;
			...
			case 3: // south
				...
				for(uint i=0;i<size;i++)
				{
					mtc->push_back(metric_y[metric_index[p]+i]);
				}
				break;
			case 4: // north
				op=4;
			...
			data->p = mtc;
			IrisEvent* event = new IrisEvent();
			event->type = DBAR_ARRIVAL_EVENT; //TODO: merge to LINK_ARRIVAL_EVENT DBAR_ARRIVAL_EVENT
			...
			//char c[]=str.str();
			_DBG("; do_dbar(); %s", "metric_x");
			cerr << str_x.str();
			_DBG("; do_dbar(); %s", "metric_y");
			cerr << str_y.str();
			cerr <<endl;
#endif
		}
...
void
RtVcMPmDBAR::do_input_buffering()
{
	for( uint i=0; i<ports*vcs; i++)
	{
...
void
RtVcMPmDBAR::do_switch_traversal()
{
#ifdef _DEBUG_ROUTER
	for( uint i=0; i<ports; i++)
	{
...
void
RtVcMPmDBAR::do_switch_allocation()
{
...
void
RtVcMPmDBAR::do_switch_allocation_m() //Yi: m=modified
{
	bool was_swa_active = false; /* flag to make sure swa accesses are not double counted */
...
void
RtVcMPmDBAR::do_virtual_channel_allocation()
{
	/* flag to count accesses. In effect multiple w
	 * inners can be chosen for one access of the arbiter */
	bool was_vca_active = false;
...
void
RtVcMPmDBAR::request_switch_allocation()
{
	for( uint i=0; i<(ports*vcs); i++)
...
void
RtVcMPmDBAR::handle_tick_event ( IrisEvent* e )
{
	ticking = false;
	//do_switch_traversal();
	do_st();
	...
	for( uint i=0; i<(ports*vcs); i++)
		if( input_buffer_state[i].pipe_stage == IB )
		{
				...
				if ( input_buffer_state[i].possible_oports.size() != 0 )
				{
					...
					else if(cg_metric == VIRTUAL_CHANNEL )
					{
						for(uint j=0;j< input_buffer_state[i].possible_oports.size();j++)
						{
							...
							uint current_x=node_ip%grid_size;
							uint current_y=node_ip/grid_size;
							uint dest_x=input_buffer_state[i].destination%grid_size;
							uint dest_y=input_buffer_state[i].destination/grid_size;
#ifdef  _DEBUG_DBAR
			ostringstream str_x,str_y;
			for(uint ii=0;ii<metric_x.size();ii++)
			{
				str_x << " "<< metric_x[ii]<< " ";
			}
			for(uint ii=0;ii<metric_y.size();ii++)
			{
				str_y << " "<< metric_y[ii]<< " ";
			}
			//char c[]=str.str();
			...
			//cerr <<endl;
							...
							switch(temp_op)
							{
							case 1:// op is west , check the in_buffer of east port
								p=2;
								index=1+metric_index[p];
								size=current_x-dest_x;
								for(uint t=0;t<size;t++)
								{
									temp_cg_metric +=metric_x[index+t]<<(size-1-t);
								}
								_DBG("sorted metrics VIRTUAL_CHANNEL: op_%d p_%d index_%d size_%d current_x_%d dest_x_%d", temp_op,p,index,size,current_x,dest_x);
								break;
							case 2:// op is east , check the in_buffer of west port
								p=1;
								index=1+metric_index[p];
								size=dest_x-current_x;
								for(uint t=0;t<size;t++)
								{
									//uint a=metric_x[index+t];
									//uint b=size-1-t;
									//uint c=a<<b;
									//temp_cg_metric +=c;
									temp_cg_metric +=metric_x[index+t]<<(size-1-t);
									//temp_cg_metric +=metric_x[index+t]*(2^(size-1-t)); //wrong why (2^1=0x11=3)
								}
								_DBG("sorted metrics VIRTUAL_CHANNEL: op_%d p_%d index_%d size_%d current_x_%d dest_x_%d", temp_op,p,index,size,current_x,dest_x);
								break;
							case 3:// op is south , check the in_buffer of north port
								p=4;
								index=1+metric_index[p];
								size=current_y-dest_y;
								for(uint t=0;t<size;t++)
								{
									temp_cg_metric +=metric_y[index+t]<<(size-1-t);
								}
								_DBG("sorted metrics VIRTUAL_CHANNEL: op_%d p_%d index_%d size_%d current_y_%d dest_y_%d", temp_op,p,index,size,current_y,dest_y);
								break;
							case 4:// op is north , check the in_buffer of south port
								p=3;
								index=1+metric_index[p];
								size=dest_y-current_y;//grid_size -1 -current_y;
								for(uint t=0;t<size;t++)
								{
									temp_cg_metric +=metric_y[index+t]<<(size-1-t);
								}
							...
							metrics.push_back(pair<uint,uint>(input_buffer_state[i].possible_oports.at(j), temp_cg_metric));
							_DBG("sorted metrics VIRTUAL_CHANNEL: pair_%d %d %d", j, temp_op,temp_cg_metric);
						}
						if (metrics.size()>1)
						{
							sort_metrics_increase();
...
}		/* -----  end of function RtVcMPmDBAR::handle_input_arbitration_event  ----- */

string
RtVcMPmDBAR::toString() const
{
	stringstream str;
	str << "RtVcMPmDBAR"
			<< "\t addr: " << address
...
void
RtVcMPmDBAR::send_credit_back(uint i)
{
...
uint
RtVcMPmDBAR::get_sum_bf(uint port)
{
	uint sum_bf=0;
	for( uint j=0; j<vcs; j++)
	{
		...
		if( input_buffer_state[i].pipe_stage == IB )
		{
			...
			if ( input_buffer_state[i].possible_oports.size() == 2 )
			{
				...
				for ( uint i=0; i<no_adaptive_aports; i++ )
				{
					...
					if( static_cast<RtVcMPmDBAR*>(static_cast<GenericLink*>(output_connections[porta])->output_connection)->neighbors[rc_port] != NULL )  // should check if exits
					{
						a_aoc.push_back(rc_port);
						temp=static_cast<RtVcMPmDBAR*>(static_cast<GenericLink*>(output_connections[porta])->output_connection)->get_sum_bf(rc_port);
				...
				for ( uint i=0; i<no_adaptive_bports; i++ )
				{
					...
					if( static_cast<RtVcMPmDBAR*>(static_cast<GenericLink*>(output_connections[portb])->output_connection)->neighbors[rc_port] != NULL )  // should check if exits
					{
						b_aoc.push_back(rc_port);
						temp=static_cast<RtVcMPmDBAR*>(static_cast<GenericLink*>(output_connections[portb])->output_connection)->get_sum_bf(rc_port);

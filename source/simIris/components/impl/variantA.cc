...
#ifndef  _variantA_cc_INC
#define  _variantA_cc_INC
...
VariantA::VariantA ()
{
    name = "VariantA" ;
    ticking = false;
}  /* -----  end of method VariantA::VariantA  (constructor)  ----- */
...
void
VariantA::find_neighbors(uint p, uint v, uint cr, uint bs)
{
    ports =p;
    vcs =v;
    credits =cr;
...
void
VariantA::set_temp_opfault(uint p, double t, ullint d)
{
...
void
VariantA::set_temp_ipfault(uint p, double t, ullint d)
{
	return ;
}

void
VariantA::handle_fault_inject_event(IrisEvent* e )
{
...
void
VariantA::init (uint p, uint v, uint cr, uint bs)
{
    ports =p;
    vcs =v;
    credits =cr;
...
void
VariantA::set_grid_x_location( uint port, uint x_node, uint value)
{
...
void
VariantA::set_grid_y_location( uint port, uint y_node, uint value)
{
...
void
VariantA::process_event ( IrisEvent* e )
{
    switch(e->type)
    {
        case LINK_ARRIVAL_EVENT:
#ifdef _DEBUG_ROUTER
            _DBG("; VariantA::LINK_ARRIVAL_EVENT ; time:%f", Simulator::Now());
#endif
            handle_link_arrival_event(e);
            break;
        case TICK_EVENT:
#ifdef _DEBUG_ROUTER
            _DBG("; VariantA::TICK_EVENT ; time:%f", Simulator::Now());
#endif
            handle_tick_event(e);
            break;
...
#ifdef _DEBUG_ROUTER
            _DBG("; VariantA::RESET_STAT_EVENT time:%f", Simulator::Now());
#endif
        	handle_reset_stat_event(e);
            break;

        default:
            _DBG(";VariantA:: Unk event exception %d", e->type);
            break;
    }
    return ;
}		/* -----  end of function VariantA::process_event  ----- */

void 
VariantA::handle_detect_deadlock_event(IrisEvent* e )
{
    for ( uint i=0; i<ports; i++)
        for ( uint j=0; j<vcs; j++)
        {
...
double VariantA::get_average_packet_latency()
{
	return ((stat_total_packet_latency+0.0)/stat_packets);
}

double VariantA::get_last_flit_out_cycle()
{
	return last_flit_out_cycle;
}

double VariantA::get_flits_per_packet()
{
	return (stat_flits+0.0)/(stat_packets) ;
}

double VariantA::get_buffer_occupancy()
{
	return stat_buffer_occupancy;	
}

double VariantA::get_packets_drop()
{
	return stat_pkts_drop;
}

double VariantA::get_swa_fail_msg_ratio()
{
	return stat_swa_fail_msg_ratio;	
}

double VariantA::get_swa_load()
{
	return stat_swa_load;
}

double VariantA::get_vca_fail_msg_ratio()
{
	return stat_vca_fail_msg_ratio;
}

double VariantA::get_vca_load()
{
	return stat_vca_load;
}
 
double VariantA::get_stat_packets()
{
	return stat_packets;	
}

double VariantA::get_stat_flits()
{
	return stat_flits;
}

double VariantA::get_stat_ib_cycles()
{
	return stat_ib_cycles;	
}

double VariantA::get_stat_rc_cycles()
{
	return stat_rc_cycles;
}

double VariantA::get_stat_vca_cycles()
{
	return stat_vca_cycles;
}
        
double VariantA::get_stat_swa_cycles()
{
	return stat_swa_cycles;
}
 
double VariantA::get_stat_st_cycles()
{
...
void
VariantA::handle_link_arrival_event ( IrisEvent* e )
{
    ...
    if(data->type == FLIT_ID)
    {
        ...
        if(data->ptr->type ==HEAD)
        {
        	_DBG("; Router BW; Flit Arrival ; addr %lld; Head;",static_cast<HeadFlit*>(data->ptr)->addr);
        }
        else if(data->ptr->type ==BODY)
        {
        	_DBG("; Router BW; Flit Arrival ; addr %lld; Body;",static_cast<BodyFlit*>(data->ptr)->addr);
        }
        else if(data->ptr->type ==TAIL)
        {
        	_DBG("; Router BW; Flit Arrival ; addr %lld; Tail;",static_cast<TailFlit*>(data->ptr)->addr);
        }
#endif
    ...
    delete e;
    return ;
}		/* -----  end of function VariantA::handle_link_arrival_event  ----- */

void
VariantA::handle_reset_stat_event( IrisEvent* e)
{
    /* init the countes */
    stat_packets = 0;
    stat_flits = 0;
...
//                                _DBG("; do_input_buffering %d ft%d",inport*vcs+invc, f->type);
                ...
                if( f->type == HEAD )
                {
                    ...
                    _DBG("; IB ; HEAD IN ; addr=%lld ; dst=%d ; vn=vn%d", hf->addr, hf->dst_address, hf->vn);
#endif
...
void
VariantA::do_switch_allocation_m() //Yi: m=modified
{
    bool was_swa_active = false; /* flag to make sure swa accesses are not double counted */
...
void
VariantA::do_virtual_channel_allocation()
{
/* flag to count accesses. In effect multiple w
 * inners can be chosen for one access of the arbiter */
bool was_vca_active = false;
...
void
VariantA::request_switch_allocation()
{
    for( uint i=0; i<(ports*vcs); i++)
...
void
VariantA::handle_tick_event ( IrisEvent* e )
{
    ticking = false;
    //do_switch_traversal();
    do_st();
...
}		/* -----  end of function VariantA::handle_input_arbitration_event  ----- */

string
VariantA::toString() const
{
    stringstream str;
    str << "VariantA"
        << "\t addr: " << address
...
void
VariantA::send_credit_back(uint i)
{
        ...
        if( input_buffer_state[i].pipe_stage == IB )
        {
        	...
        	if ( input_buffer_state[i].possible_oports.size() == 2 )
        	{
                ...
                for ( uint i=0; i<no_adaptive_aports; i++ )
                {
                    ...
                    if( static_cast<VariantA*>(static_cast<GenericLink*>(output_connections[porta])->output_connection)->neighbors[rc_port] != NULL )  // should check if exits
                    {
                    	a_aoc.push_back(rc_port);
                     temp=static_cast<VariantA*>(static_cast<GenericLink*>(output_connections[porta])->output_connection)->get_sum_bf(rc_port);
                ...
                for ( uint i=0; i<no_adaptive_bports; i++ )
                {
                    ...
                    if( static_cast<VariantA*>(static_cast<GenericLink*>(output_connections[portb])->output_connection)->neighbors[rc_port] != NULL )  // should check if exits
                    {
                        b_aoc.push_back(rc_port);
                        temp=static_cast<VariantA*>(static_cast<GenericLink*>(output_connections[portb])->output_connection)->get_sum_bf(rc_port);

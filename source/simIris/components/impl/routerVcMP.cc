...
void
RouterVcMP::find_neighbors(uint p, uint v, uint cr, uint bs)
{
    ports =p;
    vcs =v;
    credits =cr;
...
void
RouterVcMP::remove_fault(uint p)
{
	fault_ports.erase(remove(fault_ports.begin(),fault_ports.end(),p),fault_ports.end());
	return ;
}
void
RouterVcMP::handle_fault_remove_event(IrisEvent* e )
{
...
void
RouterVcMP::set_temp_ipfault(uint p, double t, ullint d)
{
	return ;
}

void
RouterVcMP::set_temp_opfault(uint p, double t, ullint d)
{
...
void
RouterVcMP::handle_fault_inject_event(IrisEvent* e )
{
...
//        fault_ports[i]=0;//init no fault;//fault flag:0-nofault,1-yes
...
#ifdef _DEBUG_ROUTER
            _DBG("; RouterVcMP::LINK_ARRIVAL_EVENT ; time:%f", Simulator::Now());
#endif
            handle_link_arrival_event(e);
            break;
        case TICK_EVENT:
#ifdef _DEBUG_ROUTER
            _DBG("; RouterVcMP::TICK_EVENT ; time:%f", Simulator::Now());
#endif
            handle_tick_event(e);
            break;
...
double RouterVcMP::get_packets_drop()
{
...
RouterVcMP::print_stats()
{
	...
	for (uint i=0; i<fault_ports.size(); i++)
	{
		str << "\n fault in port: = " << fault_ports[i];
	}
    ...
    str << "\n router[" << node_ip << "] packets: " << stat_packets
        << "\n router[" << node_ip << "] total packet latency: " << stat_total_packet_latency << " (lat:pkt.tail.depart - head.arrival)"
        ...
        << "\n router[" << node_ip << "] stat_pkts_drop: " << stat_pkts_drop
...
RouterVcMP::handle_link_arrival_event ( IrisEvent* e )
{
    ...
    else if ( data->type == CREDIT_ID)
    {
        ...
        assert(downstream_credits[port][data->vc] <= buffer_size);
...
void
RouterVcMP::do_switch_allocation_m() // Yi : m=modified
{
    bool was_swa_active = false; /* flag to make sure swa accesses are not double counted */
...
void
RouterVcMP::request_switch_allocation() // called at last of do_virtual_channel_allocation()
{
    for( uint i=0; i<(ports*vcs); i++)
...
void
RouterVcMP::handle_reset_stat_event( IrisEvent* e)
{
    /* init the countes */
    stat_packets = 0;
    stat_flits = 0;
...
uint
RouterVcMP::get_sum_bf(uint port)//get sum of credits available for one output port
{
    uint sum_bf=0;
	for( uint j=0; j<vcs; j++)
        {

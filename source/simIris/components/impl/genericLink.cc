...
#ifdef _DEBUG
            _DBG("LINK : GenericLink::LINK_ARRIVAL_EVENT time:%f", Simulator::Now());
#endif
        	handle_link_arrival_event(e);
            break;
        case RESET_STAT_EVENT:
#ifdef _DEBUG
            _DBG("LINK : GenericLink::RESET_STAT_EVENT time:%f", Simulator::Now());
#endif
...
void
GenericLink::handle_reset_stat_event( IrisEvent* e)
{
    flits_passed= 0;
    credits_passed=0;
    delete e; e=NULL;
...
#ifdef _DEBUG
    _DBG("LINK : handle_link_arrival_event vc: %d src_id: %d dest: %d, data type: %d", data->vc, e->src_id, e->dst_id, data->type);
#endif

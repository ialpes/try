...
#ifndef  _variantB_cc_INC
#define  _variantB_cc_INC
...
VariantB::VariantB ()
{
    name = "VariantB" ;
    ticking = false;
}  /* -----  end of method VariantB::VariantB  (constructor)  ----- */
...
void
VariantB::find_neighbors(uint p, uint v, uint cr, uint bs)
{
    ports =p;
    vcs =v;
    credits =cr;
...
void
VariantB::set_temp_fault(uint p,uint type)
{
...
void
VariantB::remove_fault(uint p,uint type)
{
	if(type==6000)
...
void
VariantB::set_temp_opfault(uint p, double t, ullint d)
{
...
void
VariantB::set_temp_ipfault(uint p, double t, ullint d)
{
    IrisEvent* event = new IrisEvent();
    event->type = FAULT_INJECT_EVENT;
...
void
VariantB::handle_fault_remove_event(IrisEvent* e )
{
...
void
VariantB::handle_fault_inject_event(IrisEvent* e )
{
#ifdef _DEBUG_FI
	if(e->src_id==6000)
	{
...
void
VariantB::init (uint p, uint v, uint cr, uint bs)
{
    ports =p;
    vcs =v;
    credits =cr;
...
void
VariantB::set_grid_x_location( uint port, uint x_node, uint value)
{
...
void
VariantB::set_grid_y_location( uint port, uint y_node, uint value)
{
...
void
VariantB::process_event ( IrisEvent* e )
{
    switch(e->type)
    {
        case LINK_ARRIVAL_EVENT:
#ifdef _DEBUG_ROUTER
        	_DBG_NOARG("; VariantB::LINK_ARRIVAL_EVENT ;");
#endif
            handle_link_arrival_event(e);
            break;
        case TICK_EVENT:
#ifdef _DEBUG_ROUTER
        	_DBG_NOARG("; VariantB::TICK_EVENT ;");
#endif
            handle_tick_event(e);
            break;
        case CHECK_EVENT:
#ifdef _DEBUG_ROUTER
        	_DBG_NOARG("; VariantB::CHECK_EVENT ;");
#endif
        	handle_check_event(e);
        	break;
        case DETECT_DEADLOCK_EVENT:
#ifdef _DEBUG_ROUTER
        	_DBG_NOARG("; VariantB::DETECT_DEADLOCK_EVENT ;");
#endif
            handle_detect_deadlock_event(e);
            break;
        case FAULT_INJECT_EVENT:
#ifdef _DEBUG_ROUTER
        	_DBG_NOARG("; VariantB::FAULT_INJECT_EVENT ;");
#endif
            handle_fault_inject_event(e);
            break;
        case FAULT_REMOVE_EVENT:
#ifdef _DEBUG_ROUTER
        	_DBG_NOARG("; VariantB::FAULT_REMOVE_EVENT ;");
#endif
            handle_fault_remove_event(e);
            break;
        case RESET_STAT_EVENT:
#ifdef _DEBUG_ROUTER
        	_DBG_NOARG("; VariantB::RESET_STAT_EVENT ;");
#endif
        	handle_reset_stat_event(e);
            break;

        default:
            _DBG(";VariantB:: Unk event exception %d", e->type);
            break;
    }
    return ;
}		/* -----  end of function VariantB::process_event  ----- */
void
VariantB::handle_check_event (IrisEvent* e)
{
...
void 
VariantB::handle_detect_deadlock_event(IrisEvent* e )
{
    for ( uint i=0; i<ports; i++)
        for ( uint j=0; j<vcs; j++)
        {
...
double VariantB::get_average_packet_latency()
{
	return ((stat_total_packet_latency+0.0)/stat_packets);
}

double VariantB::get_last_flit_out_cycle()
{
	return last_flit_out_cycle;
}

double VariantB::get_flits_per_packet()
{
	return (stat_flits+0.0)/(stat_packets) ;
}

double VariantB::get_buffer_occupancy()
{
	return stat_buffer_occupancy;	
}

double VariantB::get_packets_drop()
{
	return stat_pkts_drop;
}

double VariantB::get_pshf_created()
{
	return stat_pshf_created;
}
double VariantB::get_pstf_created()
{
	return stat_pstf_created;
}
double VariantB::get_swa_fail_msg_ratio()
{
	return stat_swa_fail_msg_ratio;	
}

double VariantB::get_swa_load()
{
	return stat_swa_load;
}

double VariantB::get_vca_fail_msg_ratio()
{
	return stat_vca_fail_msg_ratio;
}

double VariantB::get_vca_load()
{
	return stat_vca_load;
}
 
double VariantB::get_stat_packets()
{
	return stat_packets;	
}

double VariantB::get_stat_flits()
{
	return stat_flits;
}

double VariantB::get_stat_ib_cycles()
{
	return stat_ib_cycles;	
}

double VariantB::get_stat_rc_cycles()
{
	return stat_rc_cycles;
}

double VariantB::get_stat_vca_cycles()
{
	return stat_vca_cycles;
}
        
double VariantB::get_stat_swa_cycles()
{
	return stat_swa_cycles;
}
 
double VariantB::get_stat_st_cycles()
{
...
void
VariantB::handle_link_arrival_event ( IrisEvent* e )
{
    ...
    delete e; e=NULL;
    return ;
}		/* -----  end of function VariantB::handle_link_arrival_event  ----- */

void
VariantB::handle_reset_stat_event( IrisEvent* e)
{
    /* init the countes */
    stat_packets = 0;
    stat_flits = 0;
...
void
VariantB::do_switch_allocation_m() //Yi: m=modified
{
    bool was_swa_active = false; /* flag to make sure swa accesses are not double counted */
...
void
VariantB::do_virtual_channel_allocation()
{
/* flag to count accesses. In effect multiple w
 * inners can be chosen for one access of the arbiter */
bool was_vca_active = false;
...
void
VariantB::request_switch_allocation()
{
    for( uint i=0; i<(ports*vcs); i++)
...
void
VariantB::handle_tick_event ( IrisEvent* e )
{
    ticking = false;
    //do_switch_traversal();
    do_st();
...
}		/* -----  end of function VariantB::handle_input_arbitration_event  ----- */

string
VariantB::toString() const
{
    stringstream str;
    str << "VariantB"
        << "\t addr: " << address
...
void
VariantB::send_credit_back(uint i)
{
        ...
        if( input_buffer_state[i].pipe_stage == IB )
        {
        	...
        	if ( input_buffer_state[i].possible_oports.size() == 2 )
        	{
                ...
                for ( uint i=0; i<no_adaptive_aports; i++ )
                {
                    ...
                    if( static_cast<VariantB*>(static_cast<GenericLink*>(output_connections[porta])->output_connection)->neighbors[rc_port] != NULL )  // should check if exits
                    {
                    	a_aoc.push_back(rc_port);
                     temp=static_cast<VariantB*>(static_cast<GenericLink*>(output_connections[porta])->output_connection)->get_sum_bf(rc_port);
                ...
                for ( uint i=0; i<no_adaptive_bports; i++ )
                {
                    ...
                    if( static_cast<VariantB*>(static_cast<GenericLink*>(output_connections[portb])->output_connection)->neighbors[rc_port] != NULL )  // should check if exits
                    {
                        b_aoc.push_back(rc_port);
                        temp=static_cast<VariantB*>(static_cast<GenericLink*>(output_connections[portb])->output_connection)->get_sum_bf(rc_port);

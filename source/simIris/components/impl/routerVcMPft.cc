...
#ifndef  _routerVcMPft_cc_INC
#define  _routerVcMPft_cc_INC

#include	"routerVcMPft.h"
using namespace std;
...
RouterVcMPft::RouterVcMPft ()
{
    name = "RouterVcMPft" ;
    ticking = false;
}  /* -----  end of method RouterVcMPft::RouterVcMPft  (constructor)  ----- */
...
void
RouterVcMPft::find_neighbors(uint p, uint v, uint cr, uint bs)
{
    ports =p;
    vcs =v;
    credits =cr;
...
void
RouterVcMPft::set_temp_fault(uint p,uint type)
{
	//fault_outports[p]=1; //used as flag
	//nb_faults++;
	if(type==6000)
...
void
RouterVcMPft::remove_fault(uint p,uint type)
{
	if(type==6000)
...
void
RouterVcMPft::set_temp_opfault(uint p, double t, ullint d)
{
...
void
RouterVcMPft::set_temp_ipfault(uint p, double t, ullint d)
{
    IrisEvent* event = new IrisEvent();
    event->type = FAULT_INJECT_EVENT;
...
void
RouterVcMPft::handle_fault_remove_event(IrisEvent* e )
{
...
void
RouterVcMPft::handle_fault_inject_event(IrisEvent* e )
{
...
void
RouterVcMPft::init (uint p, uint v, uint cr, uint bs)
{
    ports =p;
    vcs =v;
    credits =cr;
...
void
RouterVcMPft::set_grid_x_location( uint port, uint x_node, uint value)
{
...
void
RouterVcMPft::set_grid_y_location( uint port, uint y_node, uint value)
{
...
void
RouterVcMPft::process_event ( IrisEvent* e )
{
...
#ifdef _DEBUG_ROUTER
            _DBG_NOARG("; RouterVcMPft::LINK_ARRIVAL_EVENT ");
#endif
            handle_link_arrival_event(e);
            break;
        case TICK_EVENT:
#ifdef _DEBUG_ROUTER
        	_DBG_NOARG("; RouterVcMPft::TICK_EVENT ");
#endif
            handle_tick_event(e);
            break;
...
#ifdef _DEBUG_ROUTER
        	_DBG_NOARG("; RouterVcMPft::FAULT_INJECT_EVENT ");
#endif
            handle_fault_inject_event(e);
            break;
        case FAULT_REMOVE_EVENT:
#ifdef _DEBUG_ROUTER
        	_DBG_NOARG("; RouterVcMPft::FAULT_REMOVE_EVENT ");
#endif
            handle_fault_remove_event(e);
            break;
        case RESET_STAT_EVENT:
#ifdef _DEBUG_ROUTER
        	_DBG_NOARG("; RouterVcMPft::RESET_STAT_EVENT ");
#endif
        	handle_reset_stat_event(e);
            break;

        default:
            _DBG(";RouterVcMPft:: Unk event exception %d", e->type);
            break;
    }
    return ;
}		/* -----  end of function RouterVcMPft::process_event  ----- */

void 
RouterVcMPft::handle_detect_deadlock_event(IrisEvent* e )
{
    for ( uint i=0; i<ports; i++)
        for ( uint j=0; j<vcs; j++)
        {
...
void 
RouterVcMPft::dump_buffer_state()
{
    for ( uint i=0; i<ports*vcs; i++)
        if(input_buffer_state[i].pipe_stage != EMPTY && input_buffer_state[i].pipe_stage != INVALID)
        {
...
double RouterVcMPft::get_average_packet_latency()
{
	return ((stat_total_packet_latency+0.0)/stat_packets);
}

double RouterVcMPft::get_last_flit_out_cycle()
{
	return last_flit_out_cycle;
}

double RouterVcMPft::get_flits_per_packet()
{
	return (stat_flits+0.0)/(stat_packets) ;
}

double RouterVcMPft::get_buffer_occupancy()
{
	return stat_buffer_occupancy;	
}

double RouterVcMPft::get_packets_drop()
{
	return stat_pkts_drop;
}

double RouterVcMPft::get_swa_fail_msg_ratio()
{
	return stat_swa_fail_msg_ratio;	
}

double RouterVcMPft::get_swa_load()
{
	return stat_swa_load;
}

double RouterVcMPft::get_vca_fail_msg_ratio()
{
	return stat_vca_fail_msg_ratio;
}

double RouterVcMPft::get_vca_load()
{
	return stat_vca_load;
}
 
double RouterVcMPft::get_stat_packets()
{
	return stat_packets;	
}

double RouterVcMPft::get_stat_flits()
{
	return stat_flits;
}

double RouterVcMPft::get_stat_ib_cycles()
{
	return stat_ib_cycles;	
}

double RouterVcMPft::get_stat_rc_cycles()
{
	return stat_rc_cycles;
}

double RouterVcMPft::get_stat_vca_cycles()
{
	return stat_vca_cycles;
}
        
double RouterVcMPft::get_stat_swa_cycles()
{
	return stat_swa_cycles;
}
 
double RouterVcMPft::get_stat_st_cycles()
{
...
void
RouterVcMPft::handle_link_arrival_event ( IrisEvent* e )
{
    ...
    delete e;
    return ;
}		/* -----  end of function RouterVcMPft::handle_link_arrival_event  ----- */

void
RouterVcMPft::handle_reset_stat_event( IrisEvent* e)
{
    /* init the countes */
    stat_packets = 0;
    stat_flits = 0;
...
void
RouterVcMPft::do_input_buffering()
{
    for( uint i=0; i<ports*vcs; i++) 
    {
...
//                _DBG("; IB ; do_input_buffering ; address ; %lld ; inport_%d*vcs+invc_%d=%d ft(f->type)=%d",input_buffer_state[i].address, inport,invc,inport*vcs+invc, f->type);
                ...
                if( f->type == HEAD )
                {
                	...
                	else // found inport_fault
                	{
                		/* TODO: in which case, headflit is dropped ? how to make sure there is no payload arrived */
                		Flit* f = in_buffers[inport].pull();
                		//stat_pkts_drop++;
                		_DBG("; Fault found during IB Drop Flit Head of %lld at port:%d",static_cast<HeadFlit*>(f)->addr, inport);
                		//send_credit_back(inport);
                	}
                }
...
void
RouterVcMPft::do_switch_traversal()
{
#ifdef _DEBUG_ROUTER
	for( uint i=0; i<ports; i++)
	{
...
void
RouterVcMPft::do_switch_allocation()
{
...
void
RouterVcMPft::do_switch_allocation_m() //Yi: m=modified 
{
    bool was_swa_active = false; /* flag to make sure swa accesses are not double counted */
                ...
                if(f->type != TAIL && !f->is_single_flit_pkt) // if f is not end of pkt, do SWA REQ for rest of pkt
                {
                    ...
                    if(in_buffers.at(ip).get_occupancy(ic))
                    {
                	 ...
                	 _DBG("; SWA ; REQ ; ip%d-ic%d|op%d-oc%d",ip,ic,op,oc);
                        ...
                        _DBG("; Create Pseudo Tail Flit:%lld, src_id:%d ",curr_pkt.address,event->src_id );
                        send_pseudo_tf = true;
                    	}
                    }
                    ticking = true;
...
//        		cerr << "Warning !"<<" IBS: "<<curr_pkt.address<<" PKT: "<< static_cast<TailFlit*>(data->ptr)->addr<<endl;
                ...
                if(!(send_pseudo_tf || (f->pseudo && f->type==HEAD && static_cast<HeadFlit*>(f)->src_address == address)))
                {
                	send_credit_back(i); // dont send back a credit to upstream router if a pseudo_tf is created
                }
...
//#define _DROPPKT
                        ...
                        if(f->type ==HEAD )// || f->is_single_flit_pkt )
                        {
                        	...
                        	_DBG("; ST Output Fault found with HeadFlit %lld => Redo RC",input_buffer_state[i].address);
                        ...
                        if(f->type ==BODY || f->type == TAIL )
                        {
                        	...
                        	*/
                        	...
                        	pshf->src_address = address;
                        	 _DBG("; Create Pseudo HeadFlit %lld",pshf->addr);
...
*/
                ...
                */
            } //end of  if ( downstream_credits.at(op).at(oc)
        }
...
void
RouterVcMPft::do_virtual_channel_allocation()
{
/* flag to count accesses. In effect multiple w
 * inners can be chosen for one access of the arbiter */
bool was_vca_active = false;
...
void
RouterVcMPft::request_switch_allocation() 
{
    for( uint i=0; i<(ports*vcs); i++)
...
void
RouterVcMPft::handle_tick_event ( IrisEvent* e )
{
    ticking = false;
    //do_switch_traversal();
    do_st();
...
//            for (uint u=0;u<nb_faults;u++)
            ...
            if ( input_buffer_state[i].possible_oports.size() == 2 )
            {
            	...
            	uint a = 0;
            	uint b = 0;
            	for (uint j=0;j<vcs;j++)  // maybe to use va_alloc (the number of vc available)
            	{
            		a = a+downstream_credits[porta][j];
            		b = b+downstream_credits[portb][j];
            	}
#ifdef _DEBUG_METRIC0
            	_DBG(";Select Function ; output port[%d] Credit = %d",porta,a);
            	_DBG(";Select Function ; output port[%d] Credit = %d",portb,b);
#endif
            	if (a>b)
            		op = input_buffer_state[i].possible_oports[0];
            	else
            		op = input_buffer_state[i].possible_oports[1];
            	}
                	...
                	uint b = 0;
                		...
                		a = remain_flits[porta];
                		b = remain_flits[portb];

    #ifdef _DEBUG_METRIC0
                	_DBG(";Select Function ; output port[%d] Remain Flits = %d",porta,a);
                	_DBG(";Select Function ; output port[%d] Remain Flits = %d",portb,b);
    #endif
                	if (a<=b)
                		op = input_buffer_state[i].possible_oports[0];
                	else
...
#ifdef _DEBUG_ROUTER
                _DBG("; VCA ; REQ ; %d=%d(ip_%d*vcs_%d+ic_%d,op_%d)",ip*vcs+ic,op,ip,vcs,ic,op);
#endif
        ...
        else if(f->type ==TAIL)
        {
        	_DBG("; Router ; Drop Flit TAIL ; addr %lld; Tail;",static_cast<TailFlit*>(f)->addr);
                  ...
                  {
                	  stat_pkts_drop++;
                	  _DBG("; OP Fault found during RC Drop Flit Head of %lld ", static_cast<HeadFlit*>(f)->addr);
                  }
        	      if(f->type ==TAIL || f->is_single_flit_pkt )
                  {
...
}		/* -----  end of function RouterVcMPft::handle_input_arbitration_event  ----- */

string
RouterVcMPft::toString() const
{
    stringstream str;
    str << "RouterVcMPft"
        << "\t addr: " << address
...
void
RouterVcMPft::send_credit_back(uint i)
{
...
uint
RouterVcMPft::get_sum_bf(uint port)
{
    uint sum_bf=0;
	for( uint j=0; j<vcs; j++)
        {
        ...
        if( input_buffer_state[i].pipe_stage == IB )
        {
        	...
        	if ( input_buffer_state[i].possible_oports.size() == 2 )
        	{
                ...
                for ( uint i=0; i<no_adaptive_aports; i++ )
                {
                    ...
                    if( static_cast<RouterVcMPft*>(static_cast<GenericLink*>(output_connections[porta])->output_connection)->neighbors[rc_port] != NULL )  // should check if exits
                    {
                    	a_aoc.push_back(rc_port);
                     temp=static_cast<RouterVcMPft*>(static_cast<GenericLink*>(output_connections[porta])->output_connection)->get_sum_bf(rc_port);
                ...
                for ( uint i=0; i<no_adaptive_bports; i++ )
                {
                    ...
                    if( static_cast<RouterVcMPft*>(static_cast<GenericLink*>(output_connections[portb])->output_connection)->neighbors[rc_port] != NULL )  // should check if exits
                    {
                        b_aoc.push_back(rc_port);
                        temp=static_cast<RouterVcMPft*>(static_cast<GenericLink*>(output_connections[portb])->output_connection)->get_sum_bf(rc_port);

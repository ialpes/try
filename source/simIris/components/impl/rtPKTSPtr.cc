...
#ifndef  _rtPKTSPtr_cc_INC
#define  _rtPKTSPtr_cc_INC
...
RtPKTSPtr::RtPKTSPtr ()
{
    name = "RtPKTSPtr" ;
    ticking = false;
}  /* -----  end of method RtPKTSPtr::RtPKTSPtr  (constructor)  ----- */
...
void
RtPKTSPtr::find_neighbors(uint p, uint v, uint cr, uint bs)
{
    ports =p;
    vcs =v;
    credits =cr;
...
void
RtPKTSPtr::set_temp_fault(uint p,uint type)
{
...
void
RtPKTSPtr::remove_fault(uint p,uint type)
{
	if(type==6000)
...
void
RtPKTSPtr::set_temp_opfault(uint p, double t, ullint d)
{
...
void
RtPKTSPtr::set_temp_ipfault(uint p, double t, ullint d)
{
    IrisEvent* event = new IrisEvent();
    event->type = FAULT_INJECT_EVENT;
...
void
RtPKTSPtr::handle_fault_remove_event(IrisEvent* e )
{
...
void
RtPKTSPtr::handle_fault_inject_event(IrisEvent* e )
{
#ifdef _DEBUG_FI
	if(e->src_id==6000)
	{
...
void
RtPKTSPtr::init (uint p, uint v, uint cr, uint bs)
{
    ports =p;
    vcs =v;
    credits =cr;
...
void
RtPKTSPtr::set_grid_x_location( uint port, uint x_node, uint value)
{
...
void
RtPKTSPtr::set_grid_y_location( uint port, uint y_node, uint value)
{
...
void
RtPKTSPtr::process_event ( IrisEvent* e )
{
    switch(e->type)
    {
        case LINK_ARRIVAL_EVENT:
#ifdef _DEBUG_ROUTER
        	_DBG_NOARG("; RtPKTSPtr::LINK_ARRIVAL_EVENT ;");
#endif
            handle_link_arrival_event(e);
            break;
        case TICK_EVENT:
#ifdef _DEBUG_ROUTER
        	_DBG_NOARG("; RtPKTSPtr::TICK_EVENT ;");
#endif
            handle_tick_event(e);
            break;
        case DETECT_DEADLOCK_EVENT:
#ifdef _DEBUG_ROUTER
        	_DBG_NOARG("; RtPKTSPtr::DETECT_DEADLOCK_EVENT ;");
#endif
            handle_detect_deadlock_event(e);
            break;
        case FAULT_INJECT_EVENT:
#ifdef _DEBUG_ROUTER
        	_DBG_NOARG("; RtPKTSPtr::FAULT_INJECT_EVENT ;");
#endif
            handle_fault_inject_event(e);
            break;
        case FAULT_REMOVE_EVENT:
#ifdef _DEBUG_ROUTER
        	_DBG_NOARG("; RtPKTSPtr::FAULT_REMOVE_EVENT ;");
#endif
            handle_fault_remove_event(e);
            break;
        case RESET_STAT_EVENT:
#ifdef _DEBUG_ROUTER
        	_DBG_NOARG("; RtPKTSPtr::RESET_STAT_EVENT ;");
#endif
        	handle_reset_stat_event(e);
            break;

        default:
            _DBG(";RtPKTSPtr:: Unk event exception %d", e->type);
            break;
    }
    return ;
}		/* -----  end of function RtPKTSPtr::process_event  ----- */

void 
RtPKTSPtr::handle_detect_deadlock_event(IrisEvent* e )
{
    for ( uint i=0; i<ports; i++)
        for ( uint j=0; j<vcs; j++)
        {
...
double RtPKTSPtr::get_average_packet_latency()
{
	return ((stat_total_packet_latency+0.0)/stat_packets);
}

double RtPKTSPtr::get_last_flit_out_cycle()
{
	return last_flit_out_cycle;
}

double RtPKTSPtr::get_flits_per_packet()
{
	return (stat_flits+0.0)/(stat_packets) ;
}

double RtPKTSPtr::get_buffer_occupancy()
{
	return stat_buffer_occupancy;	
}

double RtPKTSPtr::get_packets_drop()
{
	return stat_pkts_drop;
}

double RtPKTSPtr::get_pshf_created()
{
	return stat_pshf_created;
}
double RtPKTSPtr::get_pstf_created()
{
	return stat_pstf_created;
}
double RtPKTSPtr::get_swa_fail_msg_ratio()
{
	return stat_swa_fail_msg_ratio;	
}

double RtPKTSPtr::get_swa_load()
{
	return stat_swa_load;
}

double RtPKTSPtr::get_vca_fail_msg_ratio()
{
	return stat_vca_fail_msg_ratio;
}

double RtPKTSPtr::get_vca_load()
{
	return stat_vca_load;
}
 
double RtPKTSPtr::get_stat_packets()
{
	return stat_packets;	
}

double RtPKTSPtr::get_stat_flits()
{
	return stat_flits;
}

double RtPKTSPtr::get_stat_ib_cycles()
{
	return stat_ib_cycles;	
}

double RtPKTSPtr::get_stat_rc_cycles()
{
	return stat_rc_cycles;
}

double RtPKTSPtr::get_stat_vca_cycles()
{
	return stat_vca_cycles;
}
        
double RtPKTSPtr::get_stat_swa_cycles()
{
	return stat_swa_cycles;
}
 
double RtPKTSPtr::get_stat_st_cycles()
{
...
void
RtPKTSPtr::handle_link_arrival_event ( IrisEvent* e )
{
    ...
    delete e; e=NULL;
    return ;
}		/* -----  end of function RtPKTSPtr::handle_link_arrival_event  ----- */

void
RtPKTSPtr::handle_reset_stat_event( IrisEvent* e)
{
    /* init the countes */
    stat_packets = 0;
    stat_flits = 0;
...
//                                _DBG("; do_input_buffering %d ft%d",inport*vcs+invc, f->type);
                ...
                if( f->type == HEAD )
                {
                    ...
                    if(input_buffer_state[inport*vcs+f->vc].arrival_time==0)
...
void
RtPKTSPtr::do_switch_allocation()
{
...
void
RtPKTSPtr::do_switch_allocation_m() //Yi: m=modified
{
    bool was_swa_active = false; /* flag to make sure swa accesses are not double counted */
        ...
        if( input_buffer_state.at(i).pipe_stage==SW_TRAVERSAL )
        {
            ...
            if ( downstream_credits.at(op).at(oc) )
            {
                ...
                if( f->type == TAIL || f->is_single_flit_pkt )
                {
                    ...
                    input_buffer_state[i].arrival_time=0;
                }
...
#ifdef _DEBUG_FI
                    	_DBG("; ST Output Fault found with HeadFlit %lld 0x%x => Redo VCA",input_buffer_state[i].address, f);
#endif			
...
void
RtPKTSPtr::do_virtual_channel_allocation()
{
/* flag to count accesses. In effect multiple w
 * inners can be chosen for one access of the arbiter */
bool was_vca_active = false;
...
void
RtPKTSPtr::request_switch_allocation()
{
    for( uint i=0; i<(ports*vcs); i++)
...
void
RtPKTSPtr::handle_tick_event ( IrisEvent* e )
{
    ticking = false;
    //do_switch_traversal();
    do_st();
...
/*#ifdef _DEBUG_FI
        ...
        else if(f->type ==TAIL)
        {
        	_DBG("; Router ; Drop Flit TAIL ; addr %lld; 0x%x Tail;",static_cast<TailFlit*>(f)->addr,f);
                    ...
                    {
                    	if(Simulator::Now()-input_buffer_state[i].arrival_time >10) // drop
                    	{
                    		if(f->pseudo==false)
                    			stat_pkts_drop++;
#ifdef _DEBUG_FI
...
}		/* -----  end of function RtPKTSPtr::handle_input_arbitration_event  ----- */

string
RtPKTSPtr::toString() const
{
    stringstream str;
    str << "RtPKTSPtr"
        << "\t addr: " << address
...
void
RtPKTSPtr::send_credit_back(uint i)
{
...
uint
RtPKTSPtr::get_sum_bf(uint port)
{
    uint sum_bf=0;
	for( uint j=0; j<vcs; j++)
        {
        ...
        if( input_buffer_state[i].pipe_stage == IB )
        {
        	...
        	if ( input_buffer_state[i].possible_oports.size() == 2 )
        	{
                ...
                for ( uint i=0; i<no_adaptive_aports; i++ )
                {
                    ...
                    if( static_cast<RtPKTSPtr*>(static_cast<GenericLink*>(output_connections[porta])->output_connection)->neighbors[rc_port] != NULL )  // should check if exits
                    {
                    	a_aoc.push_back(rc_port);
                     temp=static_cast<RtPKTSPtr*>(static_cast<GenericLink*>(output_connections[porta])->output_connection)->get_sum_bf(rc_port);
                ...
                for ( uint i=0; i<no_adaptive_bports; i++ )
                {
                    ...
                    if( static_cast<RtPKTSPtr*>(static_cast<GenericLink*>(output_connections[portb])->output_connection)->neighbors[rc_port] != NULL )  // should check if exits
                    {
                        b_aoc.push_back(rc_port);
                        temp=static_cast<RtPKTSPtr*>(static_cast<GenericLink*>(output_connections[portb])->output_connection)->get_sum_bf(rc_port);

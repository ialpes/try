...
void Visual::create_graphml_mesh()
{
	...
	for (; i < nodes; i++)
	{
		...
		else if (rtr_type == "RouterVcMPm")
			ptr = dynamic_cast<RouterVcMPm*>(topo_ptr->routers[i]);
		else if (rtr_type == "VariantB")
			ptr = dynamic_cast<VariantB*>(topo_ptr->routers[i]);
		else if (rtr_type == "RTDBAR")
			ptr = dynamic_cast<RtVcMPmDBAR*>(topo_ptr->routers[i]);
		else
		{

...
CONGEST_METRIC cg_metric = FLIT_REMAIN; //FLIT_REMAIN BUFFER_SIZE
string cg_metric_string = "FLIT_REMAIN";
...
pkt_pattern pattern_type = RANDOM_UNI;
string pattern_type_string = "RANDOM_UNI";
...
/* Fault Inject */
/*typedef struct _FaultInjection
{
	uint node;
	uint port;
} FaultInjection;
vector<FaultInjection> faultsinjected;*/
uint FAULT_INJECTION_RATE;
uint nb_faults;
vector<uint> faulty_ipnodes;
vector<uint> faulty_inports;
vector<uint> faulty_opnodes;
vector<uint> faulty_outports;
vector<uint> temps_inject;
vector<uint> fault_dure;
vector<uint> fault_type;
uint srcone=0;
uint dstone=0;

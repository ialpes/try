...
double SIM_router_stat_energy(SIM_router_info_t *info, SIM_router_power_t *router, int print_depth, int max_avg
		, double buf_rl,double buf_wl,double c_tl,double n_arb,double nvc_read,double nvc_write, int plot_flag, double freq)
{
    ...
    if (info->sw_in_arb_model)
    {
        ...
        Eavg += SIM_arbiter_stat_energy(&router->sw_in_arb, &info->sw_in_arb_queue_info
        		, (info->n_v_channel*info->n_v_class)/2, next_depth, max_avg) * n_arb; //enrgy of one arbiter * total active arbiters
    ...
    else if (info->vc_allocator_type == TWO_STAGE_ARB && info->vc_in_arb_model && info->vc_out_arb_model)
    {
        ...
        if (info->vc_in_arb_model)
        {
            ...
            Eavg += SIM_arbiter_stat_energy(&router->vc_in_arb, &info->vc_in_arb_queue_info
            		, info->n_v_channel/2, next_depth, max_avg) * nActiveArbs;
        }
    ...
    else if (info->vc_allocator_type == VC_SELECT && info->n_v_channel > 1 && info->n_in > 1)
    {
        ...
        Eavg += SIM_array_stat_energy(&info->vc_select_buf_info, &router->vc_select_buf
        		, n_read , n_write, next_depth,  max_avg);
    }
    else
    {
